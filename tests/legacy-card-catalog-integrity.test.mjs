...
test('the complete pinned legacy card set remains represented exactly once in the runtime catalog', () => {
  ...
  const clbDmc40kNames=JSON.parse(fs.readFileSync(new URL('../reports/decks/precon-clb-dmc-40k-2026-09-09/intake.json',import.meta.url))).newNames;
  assert.equal(clbDmc40kNames.length,188);
  assert.deepEqual(intersection(clbDmc40kNames,[...starterNames,...c21Names,...c14Names,...c1516Names,...c1719Names,...c1920Names,...zncKhcNames,...afcMicNames,...vocNccNames]),[]);
  const bomNames=JSON.parse(fs.readFileSync(new URL('../reports/decks/precon-brc-onc-moc-sld-2026-09-09/intake.json',import.meta.url))).newNames;
  assert.equal(bomNames.length,139);
  assert.deepEqual(intersection(bomNames,[...starterNames,...c21Names,...c14Names,...c1516Names,...c1719Names,...c1920Names,...zncKhcNames,...afcMicNames,...vocNccNames,...clbDmc40kNames]),[]);
  const ltcCmmNames=JSON.parse(fs.readFileSync(new URL('../reports/decks/precon-ltc-cmm-2026-09-10/intake.json',import.meta.url))).newNames;
  assert.equal(ltcCmmNames.length,76);
  assert.deepEqual(intersection(ltcCmmNames,[...starterNames,...c21Names,...c14Names,...c1516Names,...c1719Names,...c1920Names,...zncKhcNames,...afcMicNames,...vocNccNames,...clbDmc40kNames,...bomNames]),[]);
  const cwwNames=JSON.parse(fs.readFileSync(new URL('../reports/decks/precon-cmm-woc-who-2026-09-10/intake.json',import.meta.url))).newNames;
  assert.equal(cwwNames.length,109);
  assert.deepEqual(intersection(cwwNames,[...starterNames,...c21Names,...c14Names,...c1516Names,...c1719Names,...c1920Names,...zncKhcNames,...afcMicNames,...vocNccNames,...clbDmc40kNames,...bomNames,...ltcCmmNames]),[]);
  const wlmNames=JSON.parse(fs.readFileSync(new URL('../reports/decks/precon-who-lcc-sld-mkc-2026-09-12/intake.json',import.meta.url))).newNames;
  assert.equal(wlmNames.length,209);
  assert.deepEqual(intersection(wlmNames,[...starterNames,...c21Names,...c14Names,...c1516Names,...c1719Names,...c1920Names,...zncKhcNames,...afcMicNames,...vocNccNames,...clbDmc40kNames,...bomNames,...ltcCmmNames,...cwwNames]),[]);
  const pomNames=JSON.parse(fs.readFileSync(new URL('../reports/decks/precon-pip-otc-m3c-2026-09-12/intake.json',import.meta.url))).newNames;
  assert.equal(pomNames.length,214);
  assert.deepEqual(intersection(pomNames,[...starterNames,...c21Names,...c14Names,...c1516Names,...c1719Names,...c1920Names,...zncKhcNames,...afcMicNames,...vocNccNames,...clbDmc40kNames,...bomNames,...ltcCmmNames,...cwwNames,...wlmNames]),[]);
  const bdfNames=JSON.parse(fs.readFileSync(new URL('../reports/decks/precon-blc-dsc-sld-drc-fic-2026-09-12/intake.json',import.meta.url))).newNames;
  assert.equal(bdfNames.length,156);
  assert.deepEqual(intersection(bdfNames,[...starterNames,...c21Names,...c14Names,...c1516Names,...c1719Names,...c1920Names,...zncKhcNames,...afcMicNames,...vocNccNames,...clbDmc40kNames,...bomNames,...ltcCmmNames,...cwwNames,...wlmNames,...pomNames]),[]);
  ...
  const c13Names=JSON.parse(fs.readFileSync(new URL('../reports/decks/precon-c13-td0-sld-2026-09-19/intake.json',import.meta.url))).newNames;
  assert.equal(c13Names.length,81);
  assert.deepEqual(intersection(c13Names,[...cslNames,...socNames]),[]);
  const frcNames=JSON.parse(fs.readFileSync(new URL('../reports/decks/precon-frc-2026-09-24/intake.json',import.meta.url))).newNames;
  assert.equal(frcNames.length,26);
  assert.deepEqual(intersection(frcNames,[...c13Names,...cslNames,...socNames]),[]);
  const legacyNames = Object.keys(legacyRaw.cards || {}).filter(name=>!starterNames.includes(name)&&!c21Names.includes(name)&&!c14Names.includes(name)&&!c1516Names.includes(name)&&!c1719Names.includes(name)&&!c1920Names.includes(name)&&!zncKhcNames.includes(name)&&!afcMicNames.includes(name)&&!vocNccNames.includes(name)&&!clbDmc40kNames.includes(name)&&!bomNames.includes(name)&&!ltcCmmNames.includes(name)&&!cwwNames.includes(name)&&!wlmNames.includes(name)&&!pomNames.includes(name)&&!bdfNames.includes(name)&&!socNames.includes(name)&&!cslNames.includes(name)&&!c13Names.includes(name)&&!frcNames.includes(name));
  ...
  for (const name of [...legacyNames,...starterNames,...c21Names,...c14Names,...c1516Names,...c1719Names,...c1920Names,...zncKhcNames,...afcMicNames,...vocNccNames,...clbDmc40kNames,...bomNames,...ltcCmmNames,...cwwNames,...wlmNames,...pomNames,...bdfNames,...socNames,...cslNames,...c13Names,...frcNames]) {
  ...
  const expectedRuntimeUnion = [...legacyNames, ...starterNames, ...c21Names, ...c14Names, ...c1516Names, ...c1719Names, ...c1920Names, ...zncKhcNames, ...afcMicNames, ...vocNccNames, ...clbDmc40kNames, ...bomNames, ...ltcCmmNames, ...cwwNames, ...wlmNames, ...pomNames, ...bdfNames, ...socNames, ...cslNames, ...c13Names, ...frcNames, ...genericNames, ...sauronNames];
