...
test('runtime card art uses local WebP except the explicit API fallback list', () => {
  ...
  importedFaces.push(...JSON.parse(fs.readFileSync(new URL('../reports/decks/precon-pip-otc-m3c-2026-09-12/oracle.json',import.meta.url))).cards.flatMap(r=>r.faces||[]));
  const pomImages=JSON.parse(fs.readFileSync(new URL('../reports/decks/precon-pip-otc-m3c-2026-09-12/images.json',import.meta.url)));
  for(const token of pomImages.tokenVariants)expected.add(token.alias);
  for(const [name,alias] of Object.entries(pomImages.canonicalAliases)){expected.add(name);assert.equal(MTG.cardImageURL(name),MTG.cardImageURL(alias),name+': pinned canonical token alias');}
  importedFaces.push(...JSON.parse(fs.readFileSync(new URL('../reports/decks/precon-blc-dsc-sld-drc-fic-2026-09-12/oracle.json',import.meta.url))).cards.flatMap(r=>r.faces||[]));
  const bdfImages=JSON.parse(fs.readFileSync(new URL('../reports/decks/precon-blc-dsc-sld-drc-fic-2026-09-12/images.json',import.meta.url)));
  for(const token of bdfImages.tokenVariants)expected.add(token.alias);
  for(const [name,alias] of Object.entries(bdfImages.canonicalAliases)){expected.add(name);assert.equal(MTG.cardImageURL(name),MTG.cardImageURL(alias),name+': pinned canonical token alias');}
  for(const face of importedFaces)expected.add(face.name);
  const socOracle=JSON.parse(fs.readFileSync(new URL('../reports/decks/precon-soc-2026-09-18/oracle.json',import.meta.url)));
  for(const face of socOracle.cards.flatMap(r=>r.faces||[]))expected.add(face.name);
  const socImages=JSON.parse(fs.readFileSync(new URL('../reports/decks/precon-soc-2026-09-18/images.json',import.meta.url)));
  ...
  const finalOracle=JSON.parse(fs.readFileSync(new URL('../reports/decks/precon-c13-td0-sld-2026-09-19/oracle.json',import.meta.url)));
  for(const row of finalOracle.cards){expected.add(faceName(row.raw.name));for(const face of row.faces||[])expected.add(face.name);}
