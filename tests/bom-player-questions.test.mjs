...
// Bounded reuse of the original player-facing inspector for the 139 pinned additions.
...
const batchNames=new Set(JSON.parse(fs.readFileSync(new URL('../reports/decks/precon-brc-onc-moc-sld-2026-09-09/intake.json',import.meta.url))).newNames);
assert.equal(batchNames.size,139);
...
test('BOM: svaka karta koju igrač baci pita razumljiva pitanja i ne ruši partiju', { timeout: 300_000 }, async () => {
  const problems = [];
  let cast = 0;
  let questions = 0;
  for (const [name, def] of Object.entries(MTG.DEFS).filter(([name])=>batchNames.has(name))) {
  ...
  console.log(JSON.stringify({batchCast:cast,batchQuestions:questions}));
  assert.ok(cast > 80, `premalo karata je stvarno bačeno: ${cast}`);
  assert.ok(questions > 100, `premalo pitanja je pregledano: ${questions}`);
  assert.deepEqual(problems.slice(0, 30), [], `nečitljiva ili slomljena pitanja (${problems.length}):\n${problems.slice(0, 30).join('\n')}`);
});

test('BOM: svaka aktivirana sposobnost koju igrač koristi pita razumljiva pitanja', { timeout: 180_000 }, async () => {
  const problems = [];
  let activations = 0;
  for (const [name, def] of Object.entries(MTG.DEFS).filter(([name])=>batchNames.has(name))) {
  ...
  console.log(JSON.stringify({batchActivations:activations}));
  assert.ok(activations > 10, `premalo aktivacija: ${activations}`);
  assert.deepEqual(problems.slice(0, 30), [], `nečitljiva ili slomljena pitanja (${problems.length}):\n${problems.slice(0, 30).join('\n')}`);
});
