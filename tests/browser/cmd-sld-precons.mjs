...
import {precons} from '../../scripts/import-cmd-sld-precons.mjs';
const {chromium}=await import(process.env.PLAYWRIGHT_MODULE||'playwright');
const root=fileURLToPath(new URL('../../',import.meta.url)),remote=process.env.PRECON_BASE_URL;
const out=root+'output/precon-cmd-sld-2026-09-19/'+(remote?'production-browser':'browser');
...
try{
  for(const width of [1440,390])for(const [index,deck] of precons.entries()){
    ...
    await page.goto(base);assert.equal(await page.evaluate(()=>JSON.parse(render_game_to_text()).deckCount),160);
    await page.locator('[data-menu-action="solo"]').first().click();await page.waitForSelector('.deckentry:visible',{timeout:30000});
    assert.equal(await page.evaluate(()=>Object.keys(MTG.DECKS).length),160);
