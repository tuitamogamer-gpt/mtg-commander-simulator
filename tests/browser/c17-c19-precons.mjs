...
const root=new URL('../../',import.meta.url).pathname,out=root+'output/precon-c17-c19-2026-09-08/browser';
...
try{
  for(const width of [1440,390])for(const [index,deck] of precons.entries()){
    ...
    await page.goto(base);assert.equal(await page.evaluate(()=>JSON.parse(render_game_to_text()).deckCount),62);
    await page.locator('[data-menu-action="solo"]').first().click();await page.waitForSelector('.deckentry',{timeout:30000});
    assert.equal(await page.evaluate(()=>Object.keys(MTG.DECKS).length),62);
    ...
    const opponent=precons[(index+1)%10].name;await page.locator('.botfields .deckselect').selectOption(opponent);
    ...
    await page.evaluate(({deck,opponent})=>{
      ...
      g.recalc();window._game=g;window._ui=ui;window.__c1719={done:false,error:null,commander};ui.render();
      void(async()=>{
        ...
        await g.flushTriggers();await g.priorityRound(you);__c1719.done=true;ui.render();
      })().catch(e=>{__c1719.error=e.stack;});
    },{deck,opponent});
    let stackSeen=false;
    for(let n=0;n<100;n++){
      const state=await page.evaluate(()=>({done:__c1719.done,error:__c1719.error,type:_ui.pending?.q.type,
    ...
    const state=await page.evaluate(()=>({done:__c1719.done,error:__c1719.error,zone:__c1719.commander.zone,
      manaSpent:__c1719.commander.castMeta?.manaSpent,graveyard:_game.players[0].graveyard.length,
