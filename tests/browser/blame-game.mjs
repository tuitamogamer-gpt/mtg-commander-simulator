...
// Use different lists in each seat, then swap them to exercise Blame Game as AI.
const precons = [
  {name:'Blame Game', commander:'Nelly Borca, Impulsive Accuser', slug:'blame-game'},
  {name:'Deep Clue Sea', commander:'Morska, Undersea Sleuth', slug:'deep-clue-sea'},
];
const {chromium}=await import(process.env.PLAYWRIGHT_MODULE||'playwright');
const root=fileURLToPath(new URL('../../',import.meta.url)),remote=process.env.PRECON_BASE_URL;
const out=root+'output/blame-game-2026-09-19/'+(remote?'production-browser':'browser');
...
try{
  for(const width of [1440,390])for(const [index,deck] of precons.entries()){
    ...
    await page.goto(base);assert.equal(await page.evaluate(()=>JSON.parse(render_game_to_text()).deckCount),169);
    await page.locator('[data-menu-action="solo"]').first().click();await page.waitForSelector('.deckentry:visible',{timeout:30000});
    assert.equal(await page.evaluate(()=>Object.keys(MTG.DECKS).length),169);
    ...
    await page.evaluate(({deck,opponent})=>{
      ...
      for(const name of ['Grizzly Bears','Wind Drake']){const c=new MTG.CardInst(MTG.DEFS[name],you);c.zone='graveyard';you.graveyard.push(c);}
      g.recalc();window._game=g;window._ui=ui;window.__csl={done:false,error:null,commander};ui.render();
      void(async()=>{
        ...
        await g.flushTriggers();await g.priorityRound(you);__csl.done=true;ui.render();
      })().catch(e=>{__csl.error=e.stack;});
    },{deck,opponent});
    let stackSeen=false;
    for(let n=0;n<100;n++){
      const state=await page.evaluate(()=>({done:__csl.done,error:__csl.error,type:_ui.pending?.q.type,
      ...
      else if(state.type==='chooseCards'){const selection=await page.evaluate(()=>({min:_ui.pending.q.min,n:_ui.pending.sel.length}));if(selection.n<selection.min)await page.locator('.modal .cardgrid .bigcard:not(.selected)').first().click();else await page.getByRole('button',{name:/^Confirm ✓/}).click();}
    ...
    const state=await page.evaluate(()=>({done:__csl.done,error:__csl.error,zone:__csl.commander.zone,
      manaSpent:__csl.commander.castMeta?.manaSpent,graveyard:_game.players[0].graveyard.length,
    ...
    if(!state.done){console.log(JSON.stringify(await page.evaluate(()=>({type:_ui.pending?.q.type,prompt:_ui.pending?.q.prompt,buttons:[...document.querySelectorAll('button')].filter(b=>b.getBoundingClientRect().width).map(b=>b.innerText)}))));await page.screenshot({path:out+'/'+deck.slug+'-failure.png'});}
