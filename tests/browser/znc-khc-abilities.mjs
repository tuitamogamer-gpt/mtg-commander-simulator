...
import {precons} from '../../scripts/import-znc-cmr-khc-precons.mjs';
const {chromium}=await import(process.env.PLAYWRIGHT_MODULE||'playwright');
const root=new URL('../../',import.meta.url).pathname,out=root+'output/precon-znc-cmr-khc-2026-09-08/abilities';fs.mkdirSync(out,{recursive:true});
...
try{for(const width of [1440,390])for(const deck of precons){
 ...
 await page.evaluate(async deck=>{
  ...
  const land=put('Forest',a,'hand');put('Sol Ring',a,'hand');let spell;
  if(deck.name==='Arm for Battle')spell=put('Blazing Sunsteel');
  if(deck.name==='Phantom Premonition')spell=put('Tales of the Ancestors',a,'hand');
  const secondLand=deck.name==='Reap the Tides'?put('Island',a,'hand'):null;
  ...
  void(async()=>{
   if(deck.name==="Land's Wrath"){await action(land,'Play land');await g.combatPhase(a);}
   if(deck.name==='Arm for Battle'){await action(spell,'Equip');await g.combatPhase(a);}
   if(deck.name==='Reap the Tides'){await action(land,'Play land');await action(secondLand,'Play land');}
   if(deck.name==='Phantom Premonition'){
     await action(spell,'Foretell');__proof.foretellTapped=g.lands(a).filter(c=>c.tapped).length;__proof.wasFaceDown=spell.faceDown;
     g.turnNo++;await action(spell,'Play from exile');
   }
   if(deck.name==='Elven Empire')await g.combatPhase(a);
 ...
 for(let n=0;n<250;n++){
  ...
  if(s.type==='main'){
   ...
   await page.locator('.sheetacts button:not(:disabled)').filter({hasText:new RegExp(s.label)}).first().click();continue;
  }
  if(s.type==='attackers'){
 ...
 if(deck.name==="Land's Wrath"){assert.equal(state.counters['+1/+1'],1);assert.equal(state.animated,1);assert.equal(state.life,32);}
 if(deck.name==='Arm for Battle'){assert.equal(state.hand,3);assert.equal(state.tappedLands,4);assert.equal(state.life,37);}
 if(deck.name==='Reap the Tides'){assert.equal(state.landCount,27);assert.equal(state.hand,3);}
 if(deck.name==='Phantom Premonition'){assert.equal(state.foretellTapped,0);assert.equal(state.wasFaceDown,true);assert.equal(state.spellMana,2);assert.equal(state.spellZone,'graveyard');assert.ok(state.tokens.some(t=>t.name.includes('Spirit')));}
 if(deck.name==='Elven Empire'){assert.equal(state.life,38);assert.equal(state.tokens.filter(t=>t.name.includes('Elf')).length,2);}
