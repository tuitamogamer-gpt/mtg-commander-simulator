...
import {precons} from '../../scripts/import-frc-precon.mjs';
const {chromium}=await import(process.env.PLAYWRIGHT_MODULE||'playwright');
const root=fileURLToPath(new URL('../../',import.meta.url)),remote=process.env.PRECON_BASE_URL;
const out=root+'output/precon-frc-2026-09-24/'+(remote?'production-browser':'browser');
...
try{
  for(const width of [1440,390])for(const [index,deck] of precons.entries()){
    ...
    await page.goto(base);assert.equal(await page.evaluate(()=>JSON.parse(render_game_to_text()).deckCount),170);
    await page.locator('[data-menu-action="solo"]').first().click();await page.waitForSelector('.deckentry:visible',{timeout:30000});
    assert.equal(await page.evaluate(()=>Object.keys(MTG.DECKS).length),170);
    ...
    const opponent='Elven Empire';await page.locator('.botfields .deckselect').selectOption(opponent);
    ...
    await page.evaluate(({deck,opponent})=>{
      ...
      g.recalc();window._game=g;window._ui=ui;window.__frc={done:false,error:null,commander};ui.render();
      void(async()=>{
        ...
        await g.flushTriggers();await g.priorityRound(you);__frc.done=true;ui.render();
      })().catch(e=>{__frc.error=e.stack;});
    },{deck,opponent});
    let stackSeen=false;
    for(let n=0;n<100;n++){
      const state=await page.evaluate(()=>({done:__frc.done,error:__frc.error,type:_ui.pending?.q.type,
    ...
    const state=await page.evaluate(()=>({done:__frc.done,error:__frc.error,zone:__frc.commander.zone,
      manaSpent:__frc.commander.castMeta?.manaSpent,graveyard:_game.players[0].graveyard.length,
