...
const out = (outputIndex >= 0 ? process.argv[outputIndex + 1] : process.env.GAME_QA_OUTPUT) || `${root}output/precon-brc-onc-moc-sld-2026-09-09/live`;
...
try {
  ...
  await host.locator('.deckentry[data-deck="Corrupting Influence"] .deckcard').click();
  ...
  const botDecks = ["Growing Threat", 'Tinker Time', 'Divine Convocation'];
