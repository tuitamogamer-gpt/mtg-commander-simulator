...
const out = process.env.PLAYER_GAMEPLAY_QA_OUTPUT || 'output/precon-brc-onc-moc-sld-2026-09-09/natural-game';
...
try {
  ...
  await page.locator('.decksearch input').fill(process.env.PLAYER_HUMAN_DECK || "Mishra's Burnished Banner");
  ...
  await page.locator('.botfields .deckselect').selectOption(process.env.PLAYER_AI_DECK || "Urza's Iron Alliance");
  ...
  for (; iterations < 1100; iterations++) {
    const s = await page.evaluate(() => {
      ...
      return {
        ...
        cast: q?.type === 'main' && q.casts?.length ? (()=>{const entry=q.casts.find(e=>e.card.is('Creature'))||q.casts[0];return {id:entry.card.iid,name:entry.card.name,zone:entry.card.zone};})() : null,
    ...
    if (s.type === 'main' && (s.land || s.cast)) {
      ...
      if(card.zone==='library')await page.locator('[data-testid="library-top-peek"]').click();
      else{await page.locator('.commandbutton').click();
