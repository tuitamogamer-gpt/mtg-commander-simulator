...
const root=new URL('../../',import.meta.url).pathname,out=root+'output/precon-c15-c16-2026-09-08/browser';
...
try{
  for(const width of [1440,390])for(const [index,deck] of precons.entries()){
    ...
    await page.evaluate(({deck,opponent})=>{
      ...
      g.recalc();window._game=g;window._ui=ui;window.__c1516={done:false,error:null,commander};ui.render();
      void(async()=>{
        ...
        await g.flushTriggers();await g.priorityRound(you);__c1516.done=true;ui.render();
      })().catch(e=>{__c1516.error=e.stack;});
    },{deck,opponent});
    let stackSeen=false;
    for(let n=0;n<100;n++){
      const state=await page.evaluate(()=>({done:__c1516.done,error:__c1516.error,type:_ui.pending?.q.type,
    ...
    const state=await page.evaluate(()=>({done:__c1516.done,error:__c1516.error,zone:__c1516.commander.zone,
      manaSpent:__c1516.commander.castMeta?.manaSpent,graveyard:_game.players[0].graveyard.length,
