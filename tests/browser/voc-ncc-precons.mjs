...
const root=new URL('../../',import.meta.url).pathname,out=root+'output/precon-voc-ncc-2026-09-09/browser';
...
try{
  for(const width of [1440,390])for(const [index,deck] of precons.entries()){
    ...
    await page.goto(base);assert.equal(await page.evaluate(()=>JSON.parse(render_game_to_text()).deckCount),90);
    await page.locator('[data-menu-action="solo"]').first().click();await page.waitForSelector('.deckentry',{timeout:30000});
    assert.equal(await page.evaluate(()=>Object.keys(MTG.DECKS).length),90);
