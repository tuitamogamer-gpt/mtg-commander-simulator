...
import {precons} from '../../scripts/import-brc-onc-moc-sld-precons.mjs';
const {chromium}=await import(process.env.PLAYWRIGHT_MODULE||'playwright');
const root=new URL('../../',import.meta.url).pathname,out=root+'output/precon-brc-onc-moc-sld-2026-09-09/abilities';fs.mkdirSync(out,{recursive:true});
...
if(process.env.BOM_RESUME&&fs.existsSync(out+'/result.json'))results.push(...JSON.parse(fs.readFileSync(out+'/result.json')).results);
try{for(const width of [1440,390])for(const deck of precons){
 ...
 if(results.some(r=>r.deck===deck.name&&r.width===width))continue;
 ...
 await page.evaluate(async deck=>{
  ...
  const put=(name,p=a,zone='battlefield')=>{const c=new MTG.CardInst(MTG.DEFS[MTG.resolveDeckCardName(name)||name],p);c.zone=zone;c.sick=false;if(zone==='battlefield')g.battlefield.push(c);else p[zone].push(c);return c;};
  ...
  if(deck.name==="Mishra's Burnished Banner")spell=put('Sol Ring',a,'hand');
  if(deck.name==="Urza's Iron Alliance")spell=put('Plague Myr',a,'hand');
  if(deck.name==='Rebellion Rising')spell=put("Krenko's Command",a,'hand');
  if(deck.name==='Corrupting Influence'){spell=put('Plague Myr',a,'hand');b.poison=3;}
  if(deck.name==='Tinker Time')spell=put('Servo Schematic',a,'hand');
  if(deck.name==='Growing Threat')spell=put('Plague Myr',a,'hand');
  if(deck.name==='Divine Convocation')spell=put('Wildfire Awakener',a,'hand');
  if(deck.name==='Cavalry Charge')spell=put('Knight of the White Orchid',a,'hand');
  if(deck.name==='Call for Backup')spell=put('Conclave Sledge-Captain',a,'hand');
  if(deck.name==='From Cute to Brute')spell=put('Nissa, Vastwood Seer',a,'hand');
  ...
  void(async()=>{
   await action(spell,'Cast');
   if(deck.name==="Mishra's Burnished Banner")await g.emit('beginCombat',{player:a});
   if(["Urza's Iron Alliance",'Corrupting Influence','Tinker Time'].includes(deck.name))await g.emit('endStep',{player:a});
   if(deck.name==='Rebellion Rising'){const token=g.creatures(a).find(c=>c.isToken);token.attacking=b;await g.emit('attackedPlayer',{player:a,target:b,attackers:[{card:token,target:b}]});}
   if(deck.name==='Cavalry Charge')await g.emit('attackersDeclared',{player:a,attackers:[{card:spell,target:b}]});
   await g.flushTriggers();await g.priorityRound(a);
   if(deck.name==='Growing Threat')await action(g.bf().find(c=>c.hasSub('Incubator')),'Transform');
   if(deck.name==='From Cute to Brute'){await action(land,'Play land');await action(spell,'Reveal');}
 ...
 for(let n=0;n<250;n++){
  ...
  if(s.type==='main'){
   ...
   if(s.label==='Cast')await page.locator('.sheetacts .pbtn.primary:not(:disabled)').first().click();else await page.locator('.sheetacts button:not(:disabled)').filter({hasText:new RegExp(s.label,'i')}).first().click();continue;
  }
 ...
 if(deck.name==="Mishra's Burnished Banner")assert.ok(state.tokens.some(t=>/Warform/.test(t.name)));
 if(deck.name==="Urza's Iron Alliance")assert.ok(state.tokens.some(t=>t.subtypes.includes('Construct')&&t.power>=2));
 if(deck.name==='Rebellion Rising')assert.ok(state.tokens.some(t=>t.subtypes.includes('Goblin')));
 if(deck.name==='Corrupting Influence'){const exiled=await page.evaluate(()=>_game.players[1].exile.map(c=>({faceDown:c.meta.exileFaceDown,lookers:c.meta.exileLookers})));assert.ok(exiled.length===1);}
 if(deck.name==='Tinker Time')assert.ok(state.tokens.some(t=>t.subtypes.includes('Gremlin')&&t.power>=2));
 if(deck.name==='Growing Threat')assert.ok(state.tokens.some(t=>t.subtypes.includes('Phyrexian')&&t.power===2));
 if(deck.name==='Divine Convocation'){assert.ok(state.hand>=2);assert.ok(state.tokens.some(t=>t.subtypes.includes('Elemental')));}
 if(deck.name==='Cavalry Charge')assert.ok(seen.has('chooseCards'));
 if(deck.name==='Call for Backup')assert.ok(seen.has('chooseTargets'));
 if(deck.name==='From Cute to Brute')assert.equal(await page.evaluate(()=>__proof.spell.name),'Nissa, Sage Animist');
