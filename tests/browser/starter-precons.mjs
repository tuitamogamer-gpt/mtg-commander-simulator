...
import {starterPrecons} from '../../scripts/import-starter-precons.mjs';
const {chromium}=await import(process.env.PLAYWRIGHT_MODULE||'playwright');
const root=new URL('../../',import.meta.url).pathname,out=root+'output/precon-starter-2026-09-06/browser';
...
try{
  for(const width of [1440,390])for(const [index,deck] of starterPrecons.entries()){
    ...
    await page.goto(base);assert.equal(await page.evaluate(()=>JSON.parse(render_game_to_text()).deckCount),32);
    await page.locator('[data-menu-action="solo"]').first().click();await page.waitForSelector('.deckentry',{timeout:30000});
    assert.equal(await page.evaluate(()=>Object.keys(MTG.DECKS).length),32);
    ...
    const opponent=starterPrecons[(index+1)%5].name;await page.locator('.botfields .deckselect').selectOption(opponent);
    ...
    await page.evaluate(({deck,opponent})=>{
      ...
      g.recalc();window._game=g;window._ui=ui;window.__starter={done:false,error:null,commander};ui.render();
      void(async()=>{
        ...
        await g.flushTriggers();await g.priorityRound(you);__starter.done=true;ui.render();
      })().catch(e=>{__starter.error=e.stack;});
    },{deck,opponent});
    let stackSeen=false;
    for(let n=0;n<100;n++){
      const state=await page.evaluate(()=>({done:__starter.done,error:__starter.error,type:_ui.pending?.q.type,
    ...
    const state=await page.evaluate(()=>({done:__starter.done,error:__starter.error,zone:__starter.commander.zone,
      manaSpent:__starter.commander.castMeta?.manaSpent,graveyard:_game.players[0].graveyard.length,
    ...
    if(deck.name==='Grave Danger')assert.equal(state.graveyard,4);
