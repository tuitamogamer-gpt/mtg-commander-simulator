...
const out = (outputIndex >= 0 ? process.argv[outputIndex + 1] : process.env.GAME_QA_OUTPUT) || `${root}output/precon-clb-dmc-40k-2026-09-09/live`;
...
const browser = await pw[browserName].launch({ headless: true, ...(browserName==='chromium'?{channel:'chrome'}:{}) });
...
try {
  ...
  await host.locator('.deckentry[data-deck="Necron Dynasties"] .deckcard').click();
  ...
  const botDecks = ["Tyranid Swarm", 'The Ruinous Powers', 'Forces of the Imperium'];
