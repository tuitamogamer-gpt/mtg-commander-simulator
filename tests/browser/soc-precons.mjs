...
import {precons} from '../../scripts/import-soc-precons.mjs';
const {chromium}=await import(process.env.PLAYWRIGHT_MODULE||'playwright');
const root=fileURLToPath(new URL('../../',import.meta.url)),remote=process.env.PRECON_BASE_URL;
const out=root+'output/precon-soc-2026-09-18/'+(remote?'production-browser':'browser');
...
try{
  for(const width of [1440,390])for(const [index,deck] of precons.entries()){
    ...
    await page.evaluate(({deck,opponent})=>{
      ...
      g.recalc();window._game=g;window._ui=ui;window.__soc={done:false,error:null,commander};ui.render();
      void(async()=>{
        ...
        await g.flushTriggers();await g.priorityRound(you);__soc.done=true;ui.render();
      })().catch(e=>{__soc.error=e.stack;});
    },{deck,opponent});
    let stackSeen=false;
    for(let n=0;n<100;n++){
      const state=await page.evaluate(()=>({done:__soc.done,error:__soc.error,type:_ui.pending?.q.type,
    ...
    const state=await page.evaluate(()=>({done:__soc.done,error:__soc.error,zone:__soc.commander.zone,
      manaSpent:__soc.commander.castMeta?.manaSpent,graveyard:_game.players[0].graveyard.length,
