...
const out = (outputIndex >= 0 ? process.argv[outputIndex + 1] : process.env.GAME_QA_OUTPUT) || `${root}output/precon-znc-cmr-khc-2026-09-08/live`;
...
try {
  ...
  await host.locator('.deckentry[data-deck="Phantom Premonition"] .deckcard').click();
  ...
  const botDecks = ["Land's Wrath", 'Arm for Battle', 'Reap the Tides'];
