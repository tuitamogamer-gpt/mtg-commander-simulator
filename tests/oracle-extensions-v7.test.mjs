...
for(const role of ['human','ai']){
 ...
 test(`v7 ${role}: owned commander grants follow ownership through stolen control`,async()=>{
  const {game,a,b}=context(role),source=put(game,a,'V7 Owned Commander'),owned=put(game,a,'Grizzly Bears'),borrowed=put(game,b,'Grizzly Bears');owned.commander=borrowed.commander=true;owned.ctrl=b;borrowed.ctrl=a;game.recalc();assert.equal(owned.cur.extraAbilities.length,1);assert.equal(borrowed.cur.extraAbilities.length,0);const n=b.library.length;assert.equal(await game.activateAbility(b,game.activatableList(b).find(row=>row.card===owned)),true);await settle(game);assert.equal(b.library.length,n-1);await game.move(source,'graveyard');assert.equal(owned.cur.extraAbilities.length,0);
 });
 ...
 test(`v7 ${role}: mana followups resolve immediately after producing mana`,async()=>{
  ...
  const egg=put(game,a,'V7 Egg Mana'),library=a.library.length;const eggAction=game.activatableList(a).find(row=>row.card===egg);assert.ok(eggAction);assert.equal(await game.activateAbility(a,eggAction),true);assert.equal(egg.zone,'graveyard');assert.equal(a.library.length,library-1);assert.equal(a.pool.C,0);assert.equal(a.pool.U,1);assert.equal(game.stack.length,0);
 });
