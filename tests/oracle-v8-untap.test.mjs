...
for(const role of ['human','ai']){
 ...
 test(`${role}: source phasing preserves an established duration`,async()=>{
  const ctx=context(M,role),{game,a}=ctx,{source,target}=await activation(ctx);await settle(game);game.phaseOut(source);assert.equal(target.cur.cantUntap,true);game.phaseInFor(a);game.recalc();assert.equal(target.cur.cantUntap,true);
 });
}
