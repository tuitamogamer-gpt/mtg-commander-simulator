...
test('exactly the eighteen recorded native cards gain import eligibility without adding definitions or built-in decks', () => {
  ...
  assert.equal(Object.keys(M.DEFS).length, 20962); assert.equal(Object.keys(M.DECKS).length, 120);
  ...
  assert.equal(Object.values(M.CARD_CATALOG).filter(c => c.deckImportEligible).length, 20962);
});
