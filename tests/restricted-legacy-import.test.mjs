...
test('the eighteen reviewed native cards retain their provenance and import eligibility', () => {
  ...
  assert.equal(Object.keys(M.DEFS).length, 3949 + genericCount); assert.equal(Object.keys(M.DECKS).length, 169);
  ...
  assert.equal(Object.values(M.CARD_CATALOG).filter(c => c.deckImportEligible).length, 3948 + genericCount);
});
