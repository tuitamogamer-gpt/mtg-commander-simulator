...
test('exactly the eighteen recorded native cards gain import eligibility without adding definitions or built-in decks', () => {
  ...
  assert.equal(Object.keys(M.DEFS).length, 22589); assert.equal(Object.keys(M.DECKS).length, 153);
  ...
  assert.equal(Object.values(M.CARD_CATALOG).filter(c => c.deckImportEligible).length, 22589);
});
