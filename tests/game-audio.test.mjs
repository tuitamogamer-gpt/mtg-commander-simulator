...
test('actual public outcomes select hit, explosion, prevention and zone sounds without hidden identity', () => {
  const effect = props => cues({ type: 'gameEffect', ...props });
  assert.deepEqual(effect({ kind: 'damage', amount: 0 }), []);
  assert.deepEqual(effect({ kind: 'damage', amount: 2, combat: true }), ['impact']);
  assert.deepEqual(effect({ kind: 'damage', amount: 8, combat: true }), ['heavy-impact']);
  assert.deepEqual(effect({ kind: 'damage', amount: 5, source: { colors: ['R'] } }), ['explosion']);
  const hidden = { faceDown: true, get colors() { throw new Error('Hidden identity read'); }, get cur() { throw new Error('Hidden identity read'); } };
  assert.deepEqual(effect({ kind: 'damage', amount: 5, source: hidden }), ['bolt']);
  assert.deepEqual(cues({ type: 'battlefieldArrival', card: hidden }), []);
  assert.deepEqual(effect({ kind: 'damagePrevented', amount: 5 }), ['ward']);
  assert.deepEqual(effect({ kind: 'zoneMove', fromZone: 'battlefield', toZone: 'graveyard' }), ['death']);
  assert.deepEqual(effect({ kind: 'zoneMove', fromZone: 'battlefield', toZone: 'exile' }), ['portal']);
});
test('browser interaction unlocks lazy audio, levels persist, mute stops voices and replay stays quiet', async () => {
  ...
  f.audio.handle({ type: 'cardPlayed' }, f.game); await flush(); assert.equal(f.sources.length, 0);
  ...
  f.audio.handle({ type: 'cardPlayed' }, f.game, { replay: true });
  f.audio.handle({ type: 'cardPlayed' }, {}); await flush(); assert.equal(f.audio.history.length, 0);
  f.audio.handle({ type: 'cardPlayed' }, f.game); await flush(); assert.equal(f.audio.history.at(-1).id, 'card');
...
test('simultaneous explosion suppresses the damage/death pile-up and hidden tabs discard effects', async () => {
  ...
  for (let n = 0; n < 30; n++) {
    f.audio.handle({ type: 'gameEffect', kind: 'damage', amount: 8, combat: true }, f.game);
  ...
  f.audio.handle({ type: 'cardPlayed' }, f.game); await flush(); assert.equal(f.audio.history.length, 1);
...
test('a major effect retires a quieter tail when the voice limit is full', async () => {
  const f = audioFixture(); await f.audio.unlock(); await flush();
  for (let n = 0; n < 5; n++) await f.audio.play({ id: 'card', priority: 1 });
...
test('paid human and local-AI spells/lands emit once and damage sounds reflect prevention', async () => {
  const M = loadEngine();
  for (const ai of [false, true]) {
    ...
    await game.damageAny(card, opponent, 6, { combat: true });
    assert.equal(opponent.life, 34);
    assert.ok(events.some(event => cues(event).includes('ward')));
    assert.ok(!events.some(event => cues(event).includes('heavy-impact')));
  }
});
