...
(function () {
  ...
  P.renderCommandTable = function (game, root) {
    ...
    const targeting = this.pending?.q.type === 'chooseTargets';
    rail.classList.toggle('has-targets', targeting);
    const title = targeting ? (this.pending.q.spec?.what === 'proliferate' ? 'Choose for proliferate' : 'Choose targets')
      : stage ? 'Your response' : this.pending?.q.type === 'main' ? 'Your turn' : required ? 'Your decision' : game.gameOver ? 'Game finished' : 'At the table';
