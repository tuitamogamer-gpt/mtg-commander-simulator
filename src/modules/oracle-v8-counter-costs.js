...
(function(){
 ...
 function reservations(plan){const result=[];for(const row of plan){let same=result.find(item=>item.card===row.card&&item.kind===row.kind);if(!same){same={card:row.card,zoneVersion:row.zoneVersion,kind:row.kind,n:0};result.push(same);}same.n++;}return result;}
 ...
 MTG.OracleV8CounterCosts={compile,canPay:(ctx,info)=>!!completion(ctx,info),prepare,validate,commit,reservations};
})();
