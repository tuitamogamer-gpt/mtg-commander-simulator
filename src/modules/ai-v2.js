...
(function () {
  ...
  MTG.evaluateState = function (view, perspectivePlayerId, deckProfile) {
    ...
    const immediateWinPotential = view.winnerId === perspectivePlayerId ? 1000 :
      opponents.length === 1 && myAttack >= opponents[0].life ? 55 : clamp(comboProgress * 1.4, 0, 50);
  ...
  function forecastCombat(game, defender, attackers, assignments, initial = {}) {
    ...
    for (const step of ['first', 'normal']) {
      ...
      const amount = card => {
        ...
        return Math.max(0, game.dmgAmount(card, 'first') || game.dmgAmount(card, 'normal'));
      };
      ...
      for (const { source, target, n } of hits) {
        ...
        if (target === defender) {
          result.damage += n;
          if (source.kw('infect')) result.poison += n;
          else result.life -= n;
          if (source.commander) result.commanderDamage[source.iid] = (result.commanderDamage[source.iid] || 0) + n;
        } else {
          if (source.kw('infect') || source.kw('wither')) targetState.toughness -= n;
