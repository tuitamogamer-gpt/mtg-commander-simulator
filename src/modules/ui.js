    ...
    renderDecisionModal(g) {
      ...
      if (q.type === 'effectReview') {
        if (q.effectKind === 'resolutionRecap') {
          const recap = q.recap;
          const major = recap.impact?.level === 'major';
          ov.classList.add('resolutionrecapoverlay');
          ov.classList.add(major ? 'recapmajor' : 'recapminor');
          m.classList.add('wide', 'resolutionrecap');
          m.dataset.testid = 'resolution-recap';
          m.dataset.impact = major ? 'major' : 'minor';
          ...
          if (major && recap.impact.stats.length) {
            const stats = el('div', 'resolutionrecapstats');
            for (const stat of recap.impact.stats) stats.appendChild(el('div', '', `<strong>${esc(stat.value)}</strong><span>${esc(stat.label)}</span>`));
          ...
          const playerRows = (recap.players || []).map(player => ({text: `${player.name}: ` + [
          ...
          const groups = [
            ['Search results', recap.searches], ['Resolution', recap.details],
            ['Players', playerRows], ['Battlefield', recap.changes],
          ...
          if (recap.gameOver) next.appendChild(el('p', '', 'The match has ended. Proceed to the match result.'));
            ...
            rows('Stack · next to resolve first', recap.stack.map(name => ({text: name})), detailBody);
            rows('Triggered abilities waiting for the stack', recap.queued.map(name => ({text: name})), detailBody);
