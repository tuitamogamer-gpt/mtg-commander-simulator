...
(function(){
 ...
 G.canActivateLoyalty=function(c){if(c.is('Planeswalker')&&this.bf().some(s=>C.live(s)&&s.def.oracleRulesV18?.includes('loyalty')))return false;const used=c.meta.lcLoyaltyTurn===this.turnNo?c.meta.lcLoyaltyCount||0:c.meta._loyUsed===this.turnNo?1:0;const walker=c.is('Planeswalker'),base=walker&&this.bf().some(s=>s.ctrl===c.ctrl&&C.live(s)&&s.def.lcOathTeferi)?2:1;return used<base+(walker&&c.ctrl.turnState.lcVeilTurn===this.turnNo?c.ctrl.turnState.lcVeil||0:0);};
