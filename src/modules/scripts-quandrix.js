...
(function () {
  ...
  function prepare(g, source, def) {
    ...
    g.lg(`${source.name} is prepared: ${copy.name} is ready in exile.`);
    return copy;
  }
  async function distribution(ctx, total, targets, label) {
    ...
    for (let i = 0; i < targets.length; i++) {
      ...
      const raw = await ctx.you.controller.decide(ctx.g, {
        ...
        prompt: `${label}: ${targets[i].name} (${left} remaining)`,
  ...
  async function taxCounter(ctx, so, amount) {
    ...
    if (g.canPayMana(payer, cost)) {
      const yes = await payer.controller.decide(g, { type: 'chooseOption', prompt: `Pay {${amount}} to save ${so.name}?`,
        options: [{ key: 'yes', label: 'Pay' }, { key: 'no', label: 'No' }], aiHint: { kind: 'taxCounter', amount } });
  ...
  SC['Ingenious Prodigy'] = { xCost: true, kws: ['skulk'], etbCounters: xEtb(1), triggers: [{ on: 'upkeep', opt: true, desc: 'Remove a counter and draw',
  ...
  SC['Kinetic Ooze'] = { xCost: true, etbCounters: xEtb(1), triggers: [{ on: 'etb', desc: 'X ETB', filter: etbSelf,
    targets: (g, self) => { const x = self.castMeta?.x || 0, out = [T.permanent((g2, c) => (c.is('Artifact') || c.is('Enchantment')) && c.mv <= x,
      { upTo: true, prompt: `Destroy MV ≤ ${x}`, aiHint: { goal: 'removal' } })];
      if (x >= 10) out.push(T.yourCreature({ count: 999, upTo: true, prompt: 'Double counters on other creatures',
  ...
  SC['Benevolent Hydra'] = { xCost: true, etbCounters: xEtb(1), plusCountersAdjust: (n, g, c, self) => c.ctrl === self.ctrl && c !== self ? n + 1 : n,
    abilities: [{ label: 'Counter on another creature', cost: { tap: true, rmCounter: { kind: '+1/+1', n: 1 } },
      targets: [T.yourCreature({ filter: (g, c, ctrl, src) => c.zone === 'battlefield' && c.ctrl === ctrl && c !== src, prompt: 'Another creature you control', aiHint: { goal: 'buff' } })],
  ...
  SC['Primordial Hydra'] = { xCost: true, etbCounters: xEtb(1), statics: [{ apply: (g, self) => { if ((self.counters['+1/+1'] || 0) >= 10) self.cur.kw.add('trample'); } }],
    triggers: [{ on: 'upkeep', desc: 'Double counters', filter: (g, self, d) => d.player === self.ctrl,
  ...
  async function apprentice(ctx) {
    ...
    const pick = lands.length ? await ctx.you.controller.decide(ctx.g, { type: 'chooseCards', from: lands, min: 0, max: 1, prompt: 'Land from the top 3', aiHint: { kind: 'impulseLand' } }) : [];
    ...
    while (rest.length) { const p = await ctx.you.controller.decide(ctx.g, { type: 'chooseCards', from: rest.slice(), min: 1, max: 1, prompt: 'Order on the bottom', aiHint: { kind: 'bottomOrder' } }); const c = p[0] && rest.includes(p[0]) ? p[0] : rest[0]; order.push(c); rest.splice(rest.indexOf(c), 1); }
  ...
  SC['Steelbane Hydra'] = { xCost: true, etbCounters: xEtb(1), abilities: [{ label: 'Destroy artifact/enchantment', cost: { mana: '{2}{G}', rmCounter: { kind: '+1/+1', n: 1 } },
  ...
  SC['Striding Shotcaller'] = { triggers: [{ on: 'combatDamageGroupToPlayer', desc: 'Prepare Run the Play',
    filter: (g, self, d) => d.cards.some(c => c.ctrl === self.ctrl), run: async ctx => { prepare(ctx.g, ctx.src, {
      ...
      targets: (g, c, o) => [T.creature({ count: o.xVal || 0, upTo: true, prompt: `Up to ${o.xVal || 0} creatures`, aiHint: { goal: 'buff' } })],
  ...
  SC['The Goose Mother'] = { xCost: true, etbCounters: xEtb(1), triggers: [
    { on: 'etb', desc: 'Foods', filter: etbSelf, run: async ctx => { const x = ctx.src.castMeta?.x || 0; if (x) await ctx.g.makeTokens('food', ctx.you, { n: Math.ceil(x / 2) }); } },
    { on: 'attacks', opt: true, desc: 'Food for a card', filter: (g, self, d) => d.card === self && g.bf().some(c => c.ctrl === self.ctrl && c.hasSub('Food') && g.canSacrifice(c)),
      run: async ctx => { const foods = ctx.g.bf().filter(c => c.ctrl === ctx.you && c.hasSub('Food') && ctx.g.canSacrifice(c)); const p = await ctx.you.controller.decide(ctx.g, { type: 'chooseCards', from: foods, min: 1, max: 1, prompt: 'Sacrifice a Food', aiHint: { kind: 'sacToken' } }); if (p[0]) { await ctx.g.sacrifice(ctx.you, p[0]); await ctx.g.draw(ctx.you, 1); } } },
  ] };
  ...
  SC['Lifeblood Hydra'] = { xCost: true, etbCounters: xEtb(1), triggers: [{ on: 'dies', desc: 'Life and cards', filter: (g, self, d) => d.card === self,
  ...
  SC['Nev, the Practical Dean'] = { statics: [{ apply: (g, self, bf) => { for (const c of bf) if (c.ctrl === self.ctrl && c.is('Creature') && Object.values(c.counters || {}).some(n => n > 0)) c.cur.kw.add('trample'); } }],
    triggers: [{ on: 'cast', desc: 'First X: X counters', filter: (g, self, d) => firstX(self, d), run: async ctx => { const x = ctx.data.so?.x || 0; if (x && ctx.src.zone === 'battlefield') ctx.g.addCounters(ctx.src, '+1/+1', x, false, ctx.you); } }] };
  ...
  SC['Troyan, Gutsy Explorer'] = { mana: { manual: true, cost: { tap: true }, produce: [{ G: 1, U: 1 }],
    ...
    abilities: [{ label: 'Draw, then discard', cost: { mana: '{U}', tap: true }, run: async ctx => { await ctx.g.draw(ctx.you, 1); if (ctx.you.hand.length) { const p = await ctx.you.controller.decide(ctx.g, { type: 'chooseCards', from: ctx.you.hand, min: 1, max: 1, prompt: 'Discard', aiHint: { kind: 'cleanupDiscard' } }); if (p[0]) await ctx.g.discard(ctx.you, [p[0]]); } }, aiScore: () => 1 }] };
  SC['Yavimaya Bloomsage'] = { triggers: [{ on: 'endStep', desc: '+1/+1 and Prepare Channel', filter: (g, self, d) => d.player === self.ctrl,
    targets: [T.yourCreature({ prompt: 'Creature for the counter', aiHint: { goal: 'buff' } })], run: async ctx => { const c = ctx.targets[0]; if (!c) return; ctx.g.addCounters(c, '+1/+1', 1, false, ctx.you); if (c.power < 7) return;
  ...
  SC['Altered Ego'] = { xCost: true, uncounterable: true, asEnters: async (g, card) => {
    const pool = g.creatures().filter(c => c !== card), pick = pool.length ? await card.ctrl.controller.decide(g, { type: 'chooseCards', from: pool, min: 0, max: 1, prompt: 'Altered Ego: copy', aiHint: { kind: 'mirrorCopy' } }) : [];
  ...
  SC['Forgotten Ancient'] = { triggers: [
    ...
    { on: 'upkeep', opt: true, desc: 'Move counters', filter: (g, self, d) => d.player === self.ctrl && (self.counters['+1/+1'] || 0) > 0 && g.creatures().some(c => c !== self),
      run: async ctx => { let left = ctx.src.counters['+1/+1'] || 0; const pool = ctx.g.creatures().filter(c => c !== ctx.src), pick = await ctx.you.controller.decide(ctx.g, { type: 'chooseCards', from: pool, min: 0, max: pool.length, prompt: 'Counter recipients', aiHint: { goal: 'buff' } });
        for (const c of pick) if (left > 0) { const raw = await ctx.you.controller.decide(ctx.g, { type: 'chooseX', min: 0, max: left, card: ctx.src, prompt: `Counters for ${c.name}`, aiHint: { kind: 'moveCounters', source: ctx.src, target: c } }); const n = Math.max(0, Math.min(Number(raw) || 0, left)); if (n) { ctx.g.removeCounters(ctx.src, '+1/+1', n); ctx.g.addCounters(c, '+1/+1', n, false, ctx.you); left -= n; } } } },
  ] };
  SC['Owlin Spiralmancer'] = { triggers: [{ on: 'cast', opt: true, desc: 'Copy the first X spell', filter: (g, self, d) => firstX(self, d) && d.so && g.stack.includes(d.so),
  ...
  SC['Tanazir Quandrix'] = { triggers: [
    { on: 'etb', desc: 'Double counters', filter: etbSelf, targets: [T.yourCreature({ prompt: 'Double counters', aiHint: { goal: 'buff' } })], run: async ctx => { const c = ctx.targets[0], n = c?.counters['+1/+1'] || 0; if (n) ctx.g.addCounters(c, '+1/+1', n, false, ctx.you); } },
  ...
  SC['Silkguard'] = { xCost: true,
    targets: (g, c, o) => [T.yourCreature({ count: o.xVal || 0, min: 0, upTo: true, prompt: `Up to ${o.xVal || 0} of your creatures`, aiHint: { goal: 'buff' } })],
  ...
  SC["Tyvar's Stand"] = { xCost: true, targets: [T.yourCreature({ prompt: 'Your creature', aiHint: { goal: 'buff' } })],
  ...
  SC['Quandrix Charm'] = { modes: { pick: 1, list: [
    ...
    { label: 'Base 5/5', targets: [T.creature({ prompt: 'Creature becomes base 5/5', aiHint: { goal: 'buff' } })] },
  ...
  SC["Commander's Insight"] = { xCost: true, targets: [T.player({ prompt: 'Who draws?', aiHint: { goal: 'drawSelf' } })],
  ...
  SC['Quandrix Command'] = { modes: { pick: 2, list: [
    ...
    { label: 'Two +1/+1 counters', targets: [T.creature({ prompt: 'Creature for the counters', aiHint: { goal: 'buff' } })] },
    { label: 'Up to three cards from a graveyard', targets: [
      T.player({ prompt: 'Whose graveyard?', aiHint: { goal: 'self' } }),
      {
        zone: 'graveyard', anyGraveyard: true, what: 'card', count: 3, min: 0, upTo: true,
        prompt: 'Up to three target cards from that graveyard', aiHint: { kind: 'graveyardShuffle' },
  ...
  SC['Stroke of Genius'] = { xCost: true, targets: [T.player({ prompt: 'Who draws?', aiHint: { goal: 'drawSelf' } })], resolve: async ctx => { if (ctx.targets[0]) await ctx.g.draw(ctx.targets[0], ctx.x || 0); } };
  SC['Eureka Moment'] = { resolve: async ctx => { await ctx.g.draw(ctx.you, 2); await putLand(ctx.g, ctx.you, false, 'Eureka Moment: land (optional)'); } };
  SC['Nexus Mentality'] = { modes: { pick: (g, p) => g.bf().some(c => c.ctrl === p && c.commander) ? 'any' : 1, min: 1, list: [
    { label: 'Move all counters', targets: [T.permanent((g, c, ctrl) => !c.is('Land') && c.ctrl === ctrl, { prompt: 'From which permanent?', aiHint: { goal: 'moveCountersFrom' } }), T.permanent((g, c, ctrl) => !c.is('Land') && c.ctrl === ctrl, { differentFromPrevious: true, prompt: 'To which permanent?', aiHint: { goal: 'moveCountersTo' } })] },
    { label: 'Remove counters and draw', targets: [T.permanent((g, c, ctrl) => !c.is('Land') && c.ctrl === ctrl, { prompt: 'Permanent to remove counters from', aiHint: { goal: 'drawCounters' } })] },
  ...
  SC['Perplexing Test'] = { modes: { pick: 1, list: [{ label: 'Bounce creature tokens' }, { label: 'Bounce nontoken creatures' }] },
  ...
  SC["Zimone's Hypothesis"] = { targets: [T.creature({ upTo: true, prompt: 'Optional +1/+1 counter', aiHint: { goal: 'buff' } })],
    prepareTargets: async ctx => { const k = await ctx.you.controller.decide(ctx.g, { type: 'chooseOption', prompt: 'Odd or even?', options: [{ key: 'odd', label: 'Odd' }, { key: 'even', label: 'Even' }], aiHint: { kind: 'oddEvenBounce' } }); ctx.so.quality = k === 'odd' ? 'odd' : 'even'; },
  ...
  SC['Ozolith, the Shattered Spire'] = { plusCountersAdjust: (n, g, c, self) => c.ctrl === self.ctrl && (c.is('Artifact') || c.is('Creature')) ? n + 1 : n,
    cycling: { cost: '{2}' }, abilities: [{ label: '+1/+1 counter on artifact/creature', cost: { mana: '{1}{G}', tap: true }, sorcery: true,
      targets: [T.permanent((g, c, ctrl) => c.ctrl === ctrl && (c.is('Artifact') || c.is('Creature')), { prompt: 'Your artifact/creature', aiHint: { goal: 'buff' } })],
  ...
  SC['Fractal Harness'] = { xCost: true, equip: '{2}', triggers: [
    ...
    { on: 'attacks', desc: 'Double counters', filter: (g, self, d) => self.attachedTo === d.card.iid, run: async ctx => { const c = ctx.g.byIid(ctx.src.attachedTo), n = c?.counters['+1/+1'] || 0; if (n) ctx.g.addCounters(c, '+1/+1', n, false, ctx.you); } },
  ] };
  SC['Brass Infiniscope'] = { mana: { cost: { tap: true }, produce: [{ C: 2 }], onProduce: async (g, source, p) => {
    g.delayed.push({ on: 'cast', once: true, expires: 'eot', src: source, ctrl: p, name: 'Brass Infiniscope reward',
  ...
  SC['Mana Bloom'] = { xCost: true, etbCounters: { kind: 'charge', n: (g, c) => c.castMeta?.x || 0 },
    ...
    triggers: [{ on: 'upkeep', desc: 'Return the empty Bloom', filter: (g, self, d) => d.player === self.ctrl && !(self.counters.charge || 0), run: async ctx => {
  ...
  SC['Lattice Library'] = { xCost: true, etbCounters: { kind: 'study', n: (g, c) => c.castMeta?.x || 0 }, triggers: [
    ...
    { on: 'cast', desc: 'First X: Fractal', filter: (g, self, d) => firstX(self, d), run: async ctx => { await fractal(ctx.g, ctx.you, ctx.src.counters.study || 0); } },
  ] };
  SC['Unbound Flourishing'] = { triggers: [
    { on: 'cast', desc: 'Double permanent X', filter: (g, self, d) => d.player === self.ctrl && d.so && !d.card.is('Instant') && !d.card.is('Sorcery') && isXSpell(d.card, d.so),
      run: async ctx => { const so = ctx.data.so; if (so && ctx.g.stack.includes(so)) { so.x *= 2; if (so.card.castMeta) so.card.castMeta.x = so.x; } } },
    { on: 'castIS', desc: 'Copy X spell', filter: (g, self, d) => d.player === self.ctrl && d.so && isXSpell(d.card, d.so) && g.stack.includes(d.so),
      run: async ctx => { if (ctx.g.stack.includes(ctx.data.so)) await ctx.g.copySpell(ctx.data.so, ctx.you, { mayNewTargets: true, copySource: ctx.src }); } },
    { on: 'abilityActivated', desc: 'Copy X ability', filter: (g, self, d) => d.player === self.ctrl && !d.isMana && d.stackObject && d.ability &&
  ...
  SC["Alchemist's Refuge"] = { producesColors: [], mana: { cost: { tap: true }, produce: [{ C: 1 }] }, abilities: [{ label: 'Spells have flash', cost: { mana: '{G}{U}', tap: true },
