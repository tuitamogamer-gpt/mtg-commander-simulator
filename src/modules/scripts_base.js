...
(function () {
  ...
  E.searchBasic = async function (g, p, opts = {}) {
    ...
    const got = [];
    for (let i = 0; i < n; i++) {
      const avail = (g.searchableLibrary?g.searchableLibrary(p):(g.canSearchLibrary?.(p)===false?[]:p.library)).filter(c => (c.def.super || []).includes('Basic') && (!opts.filter || opts.filter(c.def)));
