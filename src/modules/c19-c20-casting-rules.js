...
(function(){
 ...
 function offers(g,p){
  ...
  for(const r of g.c1920CastGrants||[]){if(r.vnPact)continue;const c=g.byIid(r.card);if(r.player!==p.idx||r.used||!c||c.zone!==r.zone||c.zoneVersion!==r.version||!c.owner[c.zone]?.includes(c)||r.turn!==undefined&&r.turn!==g.turnNo)continue;
   add(c,'grant:'+r.id,{...(r.exileAfter?{oracleExileOnGraveyard:true}:{}),...(r.anyColor?{asThoughAnyColor:true}:{}),...(r.anyType?{asThoughAnyType:true}:{}),label:r.label||'Cast the permitted card'},(c,a)=>!r.creature||g.castHasType(c,a,'Creature'));
  }
