((M)=>{
  ...
  function sources(game){
    const rows=new Map();
    const relevant=def=>def?.opponentGraveyardVoid||def?.c1719Slime||def?.c1920Sarcophagus||def?.c1920Rayami||def?.afcLorcan||def?.bomValentin||def?.fdcKalitas||def?.oracleZoneReplacements?.length;
