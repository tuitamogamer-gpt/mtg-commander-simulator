((M)=>{
 ...
 async function run(ctx,effect,h){
  ...
  for(const target of targets)game.untilEffects.push({kind:'oracleChosenSourcePrevention',expires:'eot',sourceCard:ctx.src,who:ctx.you,sourceRecord:selected,target,targetVersion:target?.zoneVersion,effect,consumed:false});
 }
 ...
 async function prevent(game,shield,data){
  ...
  shield.consumed=true;shield.consumedBatch=data.batch;
  if(shield.effect.after==='gain-life')await game.gainLife(shield.who,amount,shield.sourceCard);
  else if(shield.effect.after==='exile-library')for(const card of shield.who.library.slice(-amount))await game.move(card,'exile');
  return amount;
 }
