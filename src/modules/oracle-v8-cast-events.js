((M)=>{
 ...
 function triggerFilter(rule,h){
  ...
  return (game,source,data)=>{
   ...
   if(rule.kickedV17&&!kicksV17(game,so))return false;
   if(rule.optionalCostV14&&(!so.castOpts?.oracleOptionalCostV14||game.castDefinition(so.card,so.castOpts)?.oracleOptionalCostV14?.kind!==rule.optionalCostV14))return false;
   ...
   if(['library','graveyard'].includes(rule.from)&&(so.from!==rule.from||rule.zoneOwner!=='any'&&so.card.owner!==owner)||rule.from==='exile'&&so.from!=='exile')return false;
