...
export const sourceDir=path.resolve(path.dirname(fileURLToPath(import.meta.url)),'../reports/decks/precon-c17-c19-2026-09-08');
const outputDir=path.resolve(sourceDir,'../../../output/precon-c17-c19-2026-09-08');
export const precons=[
  [
    "Draconic Domination",
    "The Ur-Dragon",
    "C17",
    "HoXmEZkVqEi0kybdMDAeIw"
  ],
  [
    "Vampiric Bloodlust",
    "Edgar Markov",
    "C17",
    "dne9B6FNGk2I86ppJ3iK5Q"
  ],
  [
    "Feline Ferocity",
    "Arahbo, Roar of the World",
    "C17",
    "fKnrv6xYNky-tT7oz45nmw"
  ],
  [
    "Arcane Wizardry",
    "Inalla, Archmage Ritualist",
    "C17",
    "8c7doWGiAE6zVNPRbPryDw"
  ],
  [
    "Exquisite Invention",
    "Saheeli, the Gifted",
    "C18",
    "1fVIkyMjc0-PJiofqQgxBQ"
  ],
  [
    "Subjective Reality",
    "Aminatou, the Fateshifter",
    "C18",
    "7Eph6jxhJkekY-as0pl7XA"
  ],
  [
    "Nature's Vengeance",
    "Lord Windgrace",
    "C18",
    "m2GByoxwLU-kOIAWK3nFkw"
  ],
  [
    "Adaptive Enchantment",
    "Estrid, the Masked",
    "C18",
    "fVwQLD6CyU6YhEzQfQWzJw"
  ],
  [
    "Merciless Rage",
    "Anje Falkenrath",
    "C19",
    "z_LC4U8zVE6k4Z7EoxPV0g"
  ],
  [
    "Primal Genesis",
    "Ghired, Conclave Exile",
    "C19",
    "647BlyGrEUCXfNB-PTvCxQ"
  ]
].map(([name,commander,set,moxfield])=>({name,commander,set,moxfield,slug:name.toLowerCase().replace(/[^a-z0-9]+/g,'-')}));
const officialUrls={C17:'https://magic.wizards.com/en/news/feature/commander-2017-edition-decklists-2017-08-11',C18:'https://magic.wizards.com/en/news/feature/commander-2018-edition-decklists-and-tokens-2018-07-27',C19:'https://magic.wizards.com/en/news/feature/decks-commander-2019-edition-2019-08-08'};
...
async function sources(){
  ...
  if(official.length!==12)throw Error('Expected twelve lists in the three original editions');
  ...
  for(const row of precons){
    ...
    const json=JSON.parse(text),counts=new Map();for(const c of [...json.data.commander,...json.data.mainBoard]){const name=canonical(c.layout==='flip'?c.name.split(' // ')[0]:c.name);counts.set(name,(counts.get(name)||0)+c.count);}
  ...
  write(path.join(sourceDir,'decklists.json'),{retrievedOn:'2026-09-08',selection:'After C15/C16, all four C17 and four C18 lists, then the first two C19 lists in the pinned Moxfield index: Merciless Rage and Primal Genesis. Existing alternate editions are not imported twice.',moxfieldAccess:'The pinned index supplies queue links. These are verified Wizards / MTGJSON lists, not direct Moxfield exports.',normalization:'Budoka Gardener uses its front-face name for the flip card; the complete Scryfall faces are preserved in oracle.json.',decks});
}
async function oracle(){
  ...
  const cards=names.map(name=>{const c=byName.get(name)||byName.get(name.split(' // ')[0]);if(!c)throw Error('Missing Oracle '+name);
    if(!['normal','flip','saga'].includes(c.layout))throw Error('Unsupported layout '+c.layout+': '+name);
    const printed=c.layout==='flip'?c.card_faces[0]:c;
