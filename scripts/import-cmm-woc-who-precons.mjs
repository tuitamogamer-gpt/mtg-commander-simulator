...
export const sourceDir=path.resolve(path.dirname(fileURLToPath(import.meta.url)),'../reports/decks/precon-cmm-woc-who-2026-09-10');
export const outputDir=path.resolve(sourceDir,'../../../output/precon-cmm-woc-who-2026-09-10');
export const precons=[
  {name:'Enduring Enchantments',commander:'Anikthea, Hand of Erebos',set:'CMM',moxfield:'c6yH4J1x7kGNjygB_L_yow'},
  {name:'Eldrazi Unbound',commander:'Zhulodok, Void Gorger',set:'CMM',moxfield:'KjThzhPw-UCQBu3v3CipMA'},
  {name:'Virtue and Valor',commander:'Ellivere of the Wild Court',set:'WOC',moxfield:'KnMheGE4ykelGVpKNvopNw'},
  {name:'Fae Dominion',commander:'Tegwyll, Duke of Splendor',set:'WOC',moxfield:'ozDFoD58AkeKWefQLjSo0A'},
  {name:'Timey-Wimey',commander:'The Tenth Doctor',partner:'Rose Tyler',set:'WHO',moxfield:'fj8Av0UofkOhiyBAPSOwJw'}
].map(row=>({...row,slug:row.name.toLowerCase().replace(/[^a-z0-9]+/g,'-')}));
const officialUrls=[
  {set:'CMM',file:'cmm.html',url:'https://magic.wizards.com/en/news/announcements/commander-masters-commander-decklists'},
  {set:'WOC',file:'woc.html',url:'https://magic.wizards.com/en/news/announcements/wilds-of-eldraine-commander-decklists'},
...
const canonical=n=>n.replace(/<[^>]*>/g,'').replaceAll('Æ','Ae').replaceAll('æ','ae').replaceAll('’',"'").replace(/\s*\/{2,3}\s*/g,' // ').trim().replace(/^Dusk$/,'Dusk // Dawn').replace(/^Armont, the Redeemer$/,'Syr Armont, the Redeemer').replace(/^(.+) \/\/ \1$/,'$1');
...
async function sources(){
  ...
  write(path.join(sourceDir,'decklists.json'),{retrievedOn:'2026-09-10',selection:'After Planeswalker Party, by release order and pinned-index order within each edition: Enduring Enchantments and Eldrazi Unbound, both Wilds of Eldraine lists, then Timey-Wimey. Preserve every original card regardless of legality. No substitutions or alternate-edition duplicates.',moxfieldAccess:'The pinned index supplies queue links. These are verified Wizards / MTGJSON lists, not direct Moxfield exports.',normalization:'Flip, Adventure and double-faced cards use their front names for source comparison and resolve to existing canonical aliases during intake; split cards retain their combined names. The official Virtue and Valor list calls Syr Armont, the Redeemer simply Armont, the Redeemer; this shorthand is normalized. The WHO source has no space between the quantity and Ominous Cemetery. Time Lord is a single creature type. Full Scryfall faces are preserved in oracle.json.',decks});
}
