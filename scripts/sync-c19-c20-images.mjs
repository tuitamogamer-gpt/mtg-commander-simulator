...
import {sourceDir,outputDir} from './import-c19-c20-znc-precons.mjs';
...
const variants=[['Chandra, Flamecaller','Elemental','Chandra Elemental'],['Vraska the Unseen','Assassin','Vraska Assassin'],['Gavi, Nest Warden','Dinosaur Cat','Dinosaur Cat'],['Oona, Queen of the Fae','Faerie Rogue','Oona Faerie Rogue'],['Notorious Throng','Faerie Rogue','Notorious Faerie Rogue'],['Zaxara, the Exemplary','Hydra','C20 Hydra']];
...
const canonicalAliases = {'Faerie Rogue Token': 'Oona Faerie Rogue', 'Hydra Token': 'C20 Hydra'};
...
fs.writeFileSync(reportFile,JSON.stringify({date:'2026-09-08',source:'Pinned Scryfall card and linked token metadata',added,addedCount:added.length,canonicalAliases,sourceCardsWithLocalImages:records.length,tokenVariants:variants.map(([source,name,alias])=>({source,name,alias,scryfallId:tokens[alias].id})),existingImagesUnchanged:true},null,2)+'\n');
