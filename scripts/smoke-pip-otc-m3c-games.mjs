...
const M=loadEngine(),decks=Object.keys(M.DECKS),newDecks=JSON.parse(fs.readFileSync('reports/decks/precon-pip-otc-m3c-2026-09-12/decklists.json')).decks.map(d=>d.name),rows=[];
...
const resultFile='output/precon-pip-otc-m3c-2026-09-12/'+(previous?'final-previous-games.json':process.argv.includes('--affected-opponents')?'final-affected-games.json':'final-deck-games.json');
