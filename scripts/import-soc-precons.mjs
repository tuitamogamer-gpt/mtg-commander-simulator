...
export const sourceDir=path.resolve(path.dirname(fileURLToPath(import.meta.url)),'../reports/decks/precon-soc-2026-09-18');
export const outputDir=path.resolve(sourceDir,'../../../output/precon-soc-2026-09-18');
export const precons=[
  {
    "name": "Lorehold Spirit",
    "commander": "Quintorius, History Chaser",
    "set": "SOC",
    "moxfield": "8erwc7vsiEanV87GMM3IuQ"
  },
  {
    "name": "Silverquill Influence",
    "commander": "Killian, Decisive Mentor",
    "set": "SOC",
    "moxfield": "zQLesJzHJEyCaNMw-GUT7w"
  },
  {
    "name": "Witherbloom Pestilence",
    "commander": "Dina, Essence Brewer",
    "set": "SOC",
    "moxfield": "CIe9QwsuMUm0xHZVdXcZJQ"
  }
].map(row=>({...row,slug:row.name.toLowerCase().replace(/[^a-z0-9]+/g,'-')}));
const officialUrls=[{"set":"SOC","file":"secrets-of-strixhaven-commander-decklists.html","url":"https://magic.wizards.com/en/news/announcements/secrets-of-strixhaven-commander-decklists"}];
...
async function sources(){
  ...
  if(official.length!==5)throw Error('Expected five source lists, got '+official.length);
  ...
  for(const row of precons){
    ...
    const json=JSON.parse(text),counts=new Map();for(const c of [...json.data.commander,...json.data.mainBoard]){const name=canonical(['flip','adventure','transform','modal_dfc','prepare'].includes(c.layout)?c.name.split(' // ')[0]:c.name);counts.set(name,(counts.get(name)||0)+c.count);}
  ...
  write(path.join(sourceDir,'decklists.json'),{retrievedOn:'2026-09-18',selection:'Next three missing original lists after Revival Trance by release date and pinned index order: Lorehold Spirit, Silverquill Influence and Witherbloom Pestilence. All intervening Tarkir, Edge of Eternities, Lorwyn and Turtle Power decks, plus Prismari Artistry and Quandrix Unlimited, are already present. Collector duplicates are skipped.',moxfieldAccess:'The pinned index supplies queue links. These are verified Wizards / MTGJSON lists, not direct Moxfield exports.',normalization:"Adventure, prepare and double-faced cards use front names for source comparison; split cards retain their combined names. Quantities and punctuation are normalized for comparison. Full Scryfall faces are pinned in oracle.json.",decks});
}
