...
import {sourceDir,outputDir} from './import-frc-precon.mjs';
...
const variants=[['Ginger, Queen of Sweets','Gingerbrute Token','FRC Gingerbrute'],['Ob Nixilis, the Ascended','Angel','FRC Angel'],['Memnarch, the Warden','Myr','FRC Myr'],['Overlord of the Mistmoors','Insect','FRC Insect'],['Plan for All Outcomes','Jace','FRC Jace']];
...
for(const[source,name,alias]of variants){
 if(!tokens[alias]){const print=metadata.get(source)?.all_parts?.find(r=>r.component==='token'&&r.name.replaceAll('’',"'")===name.replaceAll('’',"'"));if(!print)throw Error('Missing linked token '+source+' / '+name);const response=await fetch(print.uri,{headers});if(!response.ok)throw Error('Token HTTP '+response.status+' '+alias);tokens[alias]=await response.json();fs.writeFileSync(tokenFile,JSON.stringify(tokens,null,2)+'\n');await new Promise(r=>setTimeout(r,120));}
...
for(const[variant,wanted]of [['normal',names],['art_crop',lists.flatMap(d=>[d.commander,'Nissa, Leyline Tamer'])]])for(const name of wanted){
...
fs.writeFileSync(reportFile,JSON.stringify({date:'2026-09-24',source:'Pinned Scryfall card and linked token metadata',added,addedCount:added.length,canonicalAliases,sourceCardsWithLocalImages:records.length,tokenVariants:variants.map(([source,name,alias])=>({source,name,alias,scryfallId:tokens[alias].id})),existingImagesUnchanged:true},null,2)+'\n');
