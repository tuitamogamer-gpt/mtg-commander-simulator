...
export const sourceDir=path.resolve(path.dirname(fileURLToPath(import.meta.url)),'../reports/decks/precon-ltc-cmm-2026-09-10');
export const outputDir=path.resolve(sourceDir,'../../../output/precon-ltc-cmm-2026-09-10');
export const precons=[
  {name:'Riders of Rohan',commander:'Éowyn, Shieldmaiden',set:'LTC',moxfield:'wWL4dez0i0euOvWSpmQ7UQ'},
  {name:'The Hosts of Mordor',commander:'Sauron, Lord of the Rings',set:'LTC',moxfield:'Z4cD-XEZRUuH4W9jB_SvuA'},
  {name:'Food and Fellowship',commander:'Frodo, Adventurous Hobbit',partner:'Sam, Loyal Attendant',set:'LTC',moxfield:'S3X49Miklk6zsQk9VSrt2Q'},
  {name:'Sliver Swarm',commander:'Sliver Gravemother',set:'CMM',moxfield:'VgocEf5MwUC9SxeIj4iXzQ'},
  {name:'Planeswalker Party',commander:'Commodore Guff',set:'CMM',moxfield:'8eidLxMRBkqRWg9yIN8nyA'}
].map(row=>({...row,slug:row.name.toLowerCase().replace(/[^a-z0-9]+/g,'-')}));
const officialUrls=[
  {set:'LTC',file:'ltc.html',url:'https://magic.wizards.com/en/news/announcements/the-lord-of-the-rings-tales-of-middle-earth-commander-decklists'},
...
const canonical=n=>n.replace(/<[^>]*>/g,'').replaceAll('Æ','Ae').replaceAll('æ','ae').replaceAll('’',"'").replace(/\s*\/{2,3}\s*/g,' // ').trim().replace(/^Dusk$/,'Dusk // Dawn').replace(/^(.+) \/\/ \1$/,'$1');
...
async function sources(){
  ...
  if(official.length!==8)throw Error('Expected eight original lists, got '+official.length);
  ...
  write(path.join(sourceDir,'decklists.json'),{retrievedOn:'2026-09-10',selection:'After From Cute to Brute, by release order and pinned-index order within each edition: three remaining LTC lists, then Sliver Swarm and Planeswalker Party. Already imported Elven Council is skipped. Preserve every original card regardless of legality. No substitutions or alternate-edition duplicates.',moxfieldAccess:'The pinned index supplies queue links. These are verified Wizards / MTGJSON lists, not direct Moxfield exports.',normalization:'Flip, Adventure and double-faced cards use their front names for source comparison and resolve to existing canonical aliases during intake; split cards retain their combined names. The official Food and Fellowship list calls Dusk // Dawn simply Dusk, so this shorthand is normalized. Full Scryfall faces are preserved in oracle.json.',decks});
}
