...
import {sourceDir,outputDir} from './import-soc-precons.mjs';
...
const variants=[['Quintorius, History Chaser','Spirit','SOC Spirit'],['Forum Filibuster','Inkling','SOC Inkling'],['Scriv, the Obligator','Contract','SOC Contract']];
...
fs.writeFileSync(reportFile,JSON.stringify({date:'2026-09-18',source:'Pinned Scryfall card and linked token metadata',added,addedCount:added.length,canonicalAliases,sourceCardsWithLocalImages:records.length,tokenVariants:variants.map(([source,name,alias])=>({source,name,alias,scryfallId:tokens[alias].id})),existingImagesUnchanged:true},null,2)+'\n');
