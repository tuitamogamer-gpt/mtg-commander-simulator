...
export const sourceDir=path.resolve(path.dirname(fileURLToPath(import.meta.url)),'../reports/decks/precon-clb-dmc-40k-2026-09-09');
export const outputDir=path.resolve(sourceDir,'../../../output/precon-clb-dmc-40k-2026-09-09');
export const precons=[
  ['Mind Flayarrrs','Captain N’ghathrod','CLB','Xv5AslbsKE-9-FPK3E3IHQ'],
  ['Party Time','Nalia de’Arnise','CLB','h1KFxvzBG0-tkfZVfBJg1A'],
  ['Draconic Dissent','Firkraag, Cunning Instigator','CLB','ZZIUxi1olEGLv284mGcMzw'],
  ['Exit from Exile','Faldorn, Dread Wolf Herald','CLB','std5mk-aL0W4JDzG3IOwbg'],
  ['Painbow','Jared Carthalion','DMC','VI61h_QqUEKf5tLvM5Pdcg'],
  ["Legends' Legacy",'Dihada, Binder of Wills','DMC','9J-00z816kymdi6vRN_zog'],
  ['Tyranid Swarm','The Swarmlord','40K','IXssBxzerEmQEWWklVWQtA'],
  ['The Ruinous Powers','Abaddon the Despoiler','40K','Co8GiCNRIUqzCJBzpdRaAg'],
  ['Necron Dynasties','Szarekh, the Silent King','40K','8ufvofa2ZkWCvXRR1lCFKQ'],
  ['Forces of the Imperium','Inquisitor Greyfax','40K','pkvfXrVXCkyG5L3OzShe2Q']
].map(([name,commander,set,moxfield])=>({name,commander:commander.replaceAll('’',"'"),set,moxfield,slug:name.toLowerCase().replace(/[^a-z0-9]+/g,'-')}));
const officialUrls=[
  {set:'CLB',file:'clb.html',url:'https://magic.wizards.com/en/news/feature/commander-legends-battle-for-baldurs-gate-commander-decklists'},
  {set:'DMC',file:'dmc.html',url:'https://magic.wizards.com/en/news/feature/dominaria-united-commander-decklists-2022-08-19'},
  {set:'40K',file:'40k.html',url:'https://magic.wizards.com/en/news/announcements/warhammer-40000-commander-decklists'}
];
...
async function sources(){
  ...
  write(path.join(sourceDir,'decklists.json'),{retrievedOn:'2026-09-09',selection:'After New Capenna, by MTGJSON release order and pinned-index order within each edition: CLB, DMC, then 40K. Preserve every original card regardless of legality. No substitutions or alternate-edition duplicates.',moxfieldAccess:'The pinned index supplies queue links. These are verified Wizards / MTGJSON lists, not direct Moxfield exports.',normalization:'Flip and Adventure cards retain their front names; split cards retain their combined names. Full Scryfall faces are preserved in oracle.json.',decks});
}
