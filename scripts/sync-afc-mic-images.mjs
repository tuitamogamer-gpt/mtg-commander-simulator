...
import {sourceDir,outputDir} from './import-afc-mic-precons.mjs';
...
full.push(...JSON.parse(fs.readFileSync(sourceDir+'/dungeons.json')).cards);
const metadata=new Map(full.flatMap(c=>[[c.name,c],...(c.card_faces||[]).map(f=>[f.name,{...c,...f,id:c.id}])]));
const variants=[['Vrondiss, Rage of Ancients','Dragon Spirit','AFC Dragon Spirit'],['Minn, Wily Illusionist','Illusion','AFC Illusion'],['Wilhelt, the Rotcleaver','Zombie','MIC Decayed Zombie'],['Tomb of Annihilation','The Atropal','The Atropal'],['Dungeon of the Mad Mage','Skeleton','AFC Skeleton'],['Lost Mine of Phandelver','Goblin','AFC Goblin']];
...
const names=new Set([...records.flatMap(r=>[r.requestedName.split(' // ')[0],...(r.faces||[]).map(f=>f.name)]),...variants.map(r=>r[2]),'Dungeon of the Mad Mage','Lost Mine of Phandelver','Tomb of Annihilation']);
...
const canonicalAliases = {'Dragon Spirit':'AFC Dragon Spirit'};
