import {extensionTarget, extensionCount, extensionCondition, extensionCost, extensionEffect, extensionLine, modifierOperation, characteristicOperation, modalOperation} from './oracle-extensions-v8.mjs';
// Version 8 preserves the v7 grammar with composable, closed leaf parsers.
// Keep the original v7 module frozen for historical manifest replay.
...
export function baseModifier(card,line) {
  const transmute=/^Transmute ((?:\{(?:\d+|[WUBRGC])\})+)$/.exec(line);
  if(transmute&&/Instant|Sorcery/.test(card.type_line)){
...
export function baseModal(card,text,parseEffect) {
...
export function baseCost(text) {
  ...
  for(const part of text.split(/,\s*/)) {
    ...
    else if((m=new RegExp('^Remove ('+NUM+') (\\+1/\\+1|-1/-1|[a-z]+) counters? from (?:this creature|this artifact|this enchantment|this land|this permanent)$','i').exec(part))){cost.rmCounter={kind:m[2],n:amount(m[1])};extended=true;}
    else return null;
  }
...
function extendedTarget(phrase) {
  ...
  if(quantity && !(quantity[1]&&quantity[2]==='one')) {
    const noun=quantity[3].replace(/\b(creatures|artifacts|enchantments|lands|permanents|planeswalkers|cards)\b/g,m=>m.slice(0,-1));
...
function extendedEffect(card,line,helpers) {
  ...
  if(/^you create /i.test(text)){
    const body=helpers.effect(card,text.slice(4)+'.');
    if(body&&!body.optional)return {...body,optional};
  }
  ...
  // Reuse an already closed single-target effect for a bounded set of
  // targets. The target descriptor carries the exact announcement range;
  // the effect still executes once for every locked object. Keep X outside
  // this adapter because its value has to be bound before targets are chosen.
  const targetRange=new RegExp('\\b(any number of |one or two |one, two, or three |up to '+NUM+' |'+NUM+' )(other )?target ([^.]+?)(?= each (?:get|gain) | to | get | gets | gain | gains | can(?:not|\\x27t)? | until |$)','i').exec(text);
  if(targetRange&&((text.match(/\btarget\b/g)||[]).length===1)){
    const targetPhrase=targetRange[0][0].toLowerCase()+targetRange[0].slice(1),target=extensionTarget(targetPhrase);
    if(target){const reduced=targetRange[0].replace(targetRange[1],targetRange[2]?'another ':'').replace(/other target/,'target').replace(/\b(creatures|artifacts|enchantments|lands|permanents|planeswalkers|cards|spells)\b/g,word=>word.slice(0,-1));const body=helpers.effect(card,text.replace(targetRange[0],reduced).replace(/to their owners' hands|to their owner's hand/g,"to its owner's hand").replace(/ each (get|gain) /g,' $1s ').replace(/ each of target /g,' target ')+'.');if(body?.targets.length===1)return {...body,targets:[target]};}
  }
  ...
  if(goad){const target=extensionTarget(goad[2]),own=new RegExp('^'+self+'$','i').test(goad[2]),filters=target||own?null:groupSelectors(goad[2]);if(target?.zone==='battlefield'||own||filters)return result([{action:goad[1].toLowerCase(),...(target||own?{target:target?0:'self'}:{filters})}],target?[target]:[]);}
  ...
  m=new RegExp('^'+self+' explores (?:(X|'+NUM+') times|twice)$','i').exec(text);
  if(m)return result([{action:'explore',target:'self',n:m[1]==='X'?'X':m[1]?amount(m[1]):2}]);
  if(new RegExp('^'+self+' explores, then (?:it|this creature) explores again$','i').test(text))return result([{action:'explore',target:'self',n:2}]);
  ...
  m=/^create a number of (.+?) tokens?( with .+?)? equal to (.+)$/i.exec(text);
  if(m){const n=extensionValue(m[3]),body=n===null?null:helpers.effect(card,'Create a '+m[1]+' token'+(m[2]||'')+'.');if(body?.effects?.length===1&&['token-inline','token-key'].includes(body.effects[0].action))return result([{...body.effects[0],n}]);}
  ...
  if(/\bthat (?:creature|artifact|land|permanent)\b/i.test(text)&&!/^target | target /i.test(text)&&!new RegExp(selfPattern(card,false),'i').test(text)){
    ...
    if(body&&!body.optional){const map=object=>Array.isArray(object)?object.map(map):object&&typeof object==='object'?Object.fromEntries(Object.entries(object).map(([key,value])=>[key,['target','otherTarget'].includes(key)&&value==='self'?'event-card':map(value)])):object;return {...body,optional:optional||body.optional,effects:body.effects.map(map)};}
  }
  ...
  m=new RegExp('^return (up to )?('+NUM+') (.+?) you control to (?:its owner\'s|their owner\'s|their owners\') hands?$','i').exec(text);
  if(m&&!/\btarget\b/.test(m[3])){const filter=extensionTarget('target '+m[3].replace(/\b(creatures|artifacts|lands|enchantments|permanents)\b/g,word=>word.slice(0,-1)));if(filter&&filter.zone==='battlefield')return result([{action:'choose-permanents',operation:'bounce',n:amount(m[2]),filter,who:'you',...(m[1]?{upTo:true}:{})}]);}
  m=new RegExp('^(untap|tap) (up to )?('+NUM+'|X) (.+)$','i').exec(text);
  if(m&&!/\btarget\b/.test(m[4])){const filter=extensionTarget('target '+m[4].replace(/\b(creatures|artifacts|lands|enchantments|permanents)\b/g,word=>word.slice(0,-1)));if(filter?.zone==='battlefield')return result([{action:'choose-permanents',operation:m[1].toLowerCase(),n:m[3]==='X'?'X':amount(m[3]),filter,who:'you',anyController:true,...(m[2]?{upTo:true}:{})}]);}
  ...
  m=/^destroy (target .+?)\. It can't be regenerated$/i.exec(text);
  if(m){const target=extensionTarget(m[1]);if(target?.zone==='battlefield')return result([{action:'destroy',target:0,noRegen:true}],[target]);}
...
function extendedLine(card,line,helpers) {
  ...
  const entryCounter='(\\+1/\\+1|-1/-1|charge|time|storage|depletion|ki|quest|spore|fuse|page|verse|bounty|muster|ice|age)';
  ...
  const counters=new RegExp('^'+self+' enters with ('+NUM+') (\\+1/\\+1|-1/-1|[a-z]+) counters? on it\\.$','i').exec(line);
...
export function baseTarget(phrase) {
...
export function baseEffect(card, line, helpers) {
  ...
  m=/^incubate (\d+)$/i.exec(text);
  if(m)return result([{action:'token-key',who:'you',n:1,tokenKey:'incubator'},{action:'counter',target:'created-tokens',counter:'+1/+1',n:Number(m[1])}]);
  ...
  m = new RegExp('^'+self+' deals ('+NUM+'|X) damage to (any target|target .+)$','i').exec(text);
  if(m) {
    const target=m[2].toLowerCase()==='any target'
      ? {what:'any',zone:'battlefield',controller:'any',min:1,max:1}
      : extensionTarget(m[2]);
    if(target) return result([{action:'damage',target:0,n:m[1]==='X'?'X':amount(m[1])}],[target]);
  }
...
export function baseLine(card, line, helpers) {
...
export function baseCondition(text) {
  ...
  if(ownCounters){
    const count={kind:'source-counters',counter:ownCounters[2]};
    if(ownCounters[1]==='no')return {kind:'count-comparison',count,max:0};
    const bound=/^(\d+) or (more|fewer)$/.exec(ownCounters[1]);
    return {kind:'count-comparison',count,[bound[2]==='more'?'min':'max']:Number(bound[1])};
  }
  const xRange=/^X is (\d+) or (more|less)$/.exec(text);
  if(xRange)return {kind:'x-range',[xRange[2]==='more'?'min':'max']:Number(xRange[1])};
  const castOrigin=/^this spell was cast from (a graveyard|your graveyard|anywhere other than your hand)$/.exec(text);
  if(castOrigin)return {kind:'cast-origin',from:castOrigin[1]==='anywhere other than your hand'?'not-hand':'graveyard'};
  const castMana=/^(\d+) or more mana was spent to cast (?:this|that) spell$/.exec(text);
  if(castMana)return {kind:'mana-total-spent',min:Number(castMana[1])};
  if(text==='two or more nonland permanents entered the battlefield under your control this turn')return {kind:'turn-stat',field:'nonlandPermanentsEntered',min:2};
  if(text==='another creature entered the battlefield under your control this turn')return {kind:'another-entry-turn',what:'creature'};
  if(text==='you descended this turn')return {kind:'turn-stat',field:'descended',min:1};
  if(/^you(?:'ve| have) cast an instant or sorcery spell this turn$/.test(text))return {kind:'cast-quality-turn',quality:'instant-or-sorcery'};
  const gravePresence=/^there(?: is|'s) (?:a|an) (.+?) card in your graveyard$/.exec(text);
  if(gravePresence){const count=extensionCount(gravePresence[1]+' cards in your graveyard');if(count)return {kind:'count-comparison',count,min:1};}
...
export function extensionValue(text) {
  const relative=/^the number of (.+?) (that player controls|in that player's (hand|graveyard|library))$/.exec(text);
  if(relative){const count=extensionCount(relative[1]+(relative[3]?' in your '+relative[3]:' you control'));if(count)return {kind:'target-count',target:'event-player',count};}
...
export function baseCharacteristic(card,line) {
