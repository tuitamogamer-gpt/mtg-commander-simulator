...
export const c21Precons=[
  {name:'Lorehold Legacies',slug:'lorehold-legacies',commander:'Osgir, the Reconstructor',moxfield:'Vgzv5-wfcUqwUbd77Flqhw'},
  {name:'Prismari Performance',slug:'prismari-performance',commander:'Zaffai, Thunder Conductor',moxfield:'aPbAlXnx1kKMj7EYicfvsg'},
  {name:'Quantum Quandrix',slug:'quantum-quandrix',commander:'Adrix and Nev, Twincasters',moxfield:'hNhQ07wNf0e7423P6S1P1g'},
  {name:'Silverquill Statement',slug:'silverquill-statement',commander:'Breena, the Demagogue',moxfield:'_dM2RHtVoUqDHjMko8X4pQ'},
  {name:'Witherbloom Witchcraft',slug:'witherbloom-witchcraft',commander:'Willowdusk, Essence Seer',moxfield:'6WeWU_rriEaCGPmJ2l1e1g'},
];
export const c21SourceDir=path.resolve(path.dirname(fileURLToPath(import.meta.url)),'../reports/decks/precon-c21-2026-09-06');
const sha=text=>crypto.createHash('sha256').update(text).digest('hex');
const cardMap=cards=>JSON.stringify(cards.map(c=>[c.name,c.n]).sort(([a],[b])=>a.localeCompare(b)));
export function buildC21Intake(M=loadEngine()){
  const official=JSON.parse(fs.readFileSync(path.join(c21SourceDir,'official-decklists.json')));
  const oracle=JSON.parse(fs.readFileSync(path.join(c21SourceDir,'oracle.json')));
  const byName=new Map(oracle.cards.map(c=>[c.requestedName,c]));
  const decks=c21Precons.map(row=>{
    const baseline=official.decks.find(d=>d.name===row.name);
    if(!baseline||baseline.commander!==row.commander)throw Error('Official commander mismatch: '+row.name);
    const filename=path.join(c21SourceDir,row.slug+'.txt');
    const text=fs.existsSync(filename)?fs.readFileSync(filename,'utf8'):null;
    const cards=text?parseMoxfieldExport(text):structuredClone(baseline.cards);
    for(const card of cards)if(!byName.has(card.name)&&byName.has(card.name.replace(' / ',' // ')))card.name=card.name.replace(' / ',' // ');
    if(cardMap(cards)!==cardMap(baseline.cards))throw Error('Moxfield / official deck mismatch: '+row.name);
    ...
    return {name:row.name,set:'C21',commander:row.commander,cards,source:{moxfield:'https://moxfield.com/decks/'+row.moxfield,
      official:official.source,provider:text?'Moxfield export verified against Wizards of the Coast':'Wizards of the Coast official decklist; Moxfield entry indexed',
...
async function refreshOracle(){
  const official=JSON.parse(fs.readFileSync(path.join(c21SourceDir,'official-decklists.json')));
  const names=[...new Set(official.decks.flatMap(d=>d.cards.map(c=>c.name)))].sort(),all=process.argv.includes('--cached-oracle')?JSON.parse(fs.readFileSync('output/precon-c21-2026-09-06/scryfall-full.json')):[];
  ...
  fs.mkdirSync('output/precon-c21-2026-09-06',{recursive:true});
  fs.writeFileSync('output/precon-c21-2026-09-06/scryfall-full.json',JSON.stringify(all,null,2));
  ...
  fs.writeFileSync(path.join(c21SourceDir,'oracle.json'),JSON.stringify({generatedAt:new Date().toISOString(),source:'https://api.scryfall.com/cards/collection',requested:names.length,found:cards.length,notFound:[],cards},null,2)+'\n');
}
async function main(){
  if(process.argv.includes('--refresh-oracle'))await refreshOracle();
  const M=loadEngine(),intake=buildC21Intake(M);
  const result={source:'Commander (2021 Edition)',retrievedOn:'2026-09-06',baselineCards:Object.keys(M.DEFS).length,
  ...
  if(process.argv.includes('--write')){
    ...
    const report=path.join(c21SourceDir,'intake.json');if(!fs.existsSync(report))fs.writeFileSync(report,JSON.stringify(result,null,2)+'\n');
  }
