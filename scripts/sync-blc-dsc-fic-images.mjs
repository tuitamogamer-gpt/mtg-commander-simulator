...
import {sourceDir,outputDir} from './import-blc-dsc-sld-drc-fic-precons.mjs';
...
const variants=[["Wizened Mentor", "Zombie", "BDF Zombie"], ["Mog, Moogle Warrior", "Moogle", "BDF Moogle"], ["Rendmaw, Creaking Nest", "Bird", "BDF Bird"], ["Demonic Covenant", "Demon", "BDF Demon"], ["Pia Nalaar, Chief Mechanic", "Nalaar Aetherjet", "BDF Nalaar Aetherjet"], ["Setzer, Wandering Gambler", "The Blackjack", "BDF The Blackjack"], ["Wrenn and Seven", "Treefolk", "BDF Treefolk"], ["Bruse Tarl, Roving Rancher", "Ox", "BDF Ox"], ["Worldspine Wurm", "Wurm", "BDF Wurm"], ["Soaring Lightbringer", "Glimmer", "BDF Glimmer"], ["Fisher's Talent", "Fish", "BDF Fish"], ["Fisher's Talent", "Shark", "BDF Shark"], ["Fisher's Talent", "Octopus", "BDF Octopus"], ["The Bears of Littjara", "Shapeshifter", "BDF Shapeshifter"], ["The Eternal Wanderer", "Samurai", "BDF Samurai"], ["Phenomenon Investigators", "Horror", "BDF Horror"], ["Perch Protection", "Bird", "BDF Bird Blue"], ["God-Eternal Oketra", "Zombie Warrior", "BDF Zombie Warrior"], ["Titania, Nature's Force", "Elemental", "BDF Elemental"]];
...
const previousAliases=fs.existsSync(sourceDir+'/images.json')?JSON.parse(fs.readFileSync(sourceDir+'/images.json')).canonicalAliases||{}:{};
...
fs.writeFileSync(reportFile,JSON.stringify({date:'2026-09-12',source:'Pinned Scryfall card and linked token metadata',added,addedCount:added.length,canonicalAliases,sourceCardsWithLocalImages:records.length,tokenVariants:variants.map(([source,name,alias])=>({source,name,alias,scryfallId:tokens[alias].id})),existingImagesUnchanged:true},null,2)+'\n');
