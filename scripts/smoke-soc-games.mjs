...
const M=loadEngine(),decks=Object.keys(M.DECKS),newDecks=JSON.parse(fs.readFileSync('reports/decks/precon-soc-2026-09-18/decklists.json')).decks.map(d=>d.name),rows=[];
...
const resultFile='output/precon-soc-2026-09-18/'+(previous?'final-previous-games.json':process.argv.includes('--affected-opponents')?'final-affected-games.json':'final-deck-games.json');
