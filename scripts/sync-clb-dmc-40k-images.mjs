...
import {sourceDir,outputDir} from './import-clb-dmc-40k-precons.mjs';
...
const undercityPath=outputDir+'/undercity-print.json';
let undercity=fs.existsSync(undercityPath)?JSON.parse(fs.readFileSync(undercityPath)):null;
if(!undercity?.id){const link=metadata.get('Seasoned Dungeoneer').all_parts.find(p=>p.name==='Undercity // The Initiative');const response=await fetch(link.uri,{headers});if(!response.ok)throw Error('Undercity metadata HTTP '+response.status);undercity=await response.json();fs.writeFileSync(undercityPath,JSON.stringify(undercity,null,2));}
metadata.set(undercity.name,undercity);
const variants=[
 ['Their Number Is Legion','Necron Warrior','40K Necron Warrior'],
 ['Birth of the Imperium','Astartes Warrior','40K Astartes Warrior'],
 ['Great Unclean One','Plaguebearer of Nurgle','Plaguebearer of Nurgle'],
 ['Bladewing, Deathless Tyrant','Zombie Knight','DMC Zombie Knight'],
 ['Cybernetica Datasmith','Robot','40K Robot'],
 ['Gargoyle Flock','Tyranid Gargoyle','40K Tyranid Gargoyle'],
 ['Hunted Horror','Centaur','CLB Centaur'],
 ['Lictor','Tyranid Warrior','40K Tyranid Warrior'],
 ['Jared Carthalion','Kavu','DMC Kavu'],
 ['Xenagos, the Reveler','Satyr','Xenagos Satyr'],
 ["Firja's Retribution",'Angel Warrior','KHM Angel Warrior'],
 ['The Lost and the Damned','Spawn','40K Spawn'],
 ['Loot Dispute','Dragon','CLB Dragon'],
 ['Kari Zev, Skyship Raider','Ragavan','Ragavan'],
 ['Pink Horror','Blue Horror','Blue Horror'],
 ["Mage's Attendant",'Wizard','CLB Wizard'],
 ['Inquisitor Eisenhorn','Cherubael','Cherubael'],
 ['Pursued Whale','Pirate','Whale Pirate'],
 ['Zellix, Sanity Flayer','Horror','CLB Horror'],
 ['Undercity // The Initiative','Skeleton','CLB Skeleton']
];
...
fs.writeFileSync(reportFile,JSON.stringify({date:'2026-09-09',source:'Pinned Scryfall card and linked token metadata',added,addedCount:added.length,canonicalAliases,sourceCardsWithLocalImages:records.length,tokenVariants:variants.map(([source,name,alias])=>({source,name,alias,scryfallId:tokens[alias].id})),existingImagesUnchanged:true},null,2)+'\n');
