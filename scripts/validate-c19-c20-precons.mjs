...
import {sourceDir,buildIntake} from './import-c19-c20-znc-precons.mjs';
const base='decdf93',sha=s=>crypto.createHash('sha256').update(s).digest('hex');
...
assert.equal(Object.keys(current.cards).length-Object.keys(previous.cards).length,149);assert.equal(current.decks.length-previous.decks.length,8);
...
assert.equal(M.CARD_CATALOG['Dockside Extortionist'].commanderLegality,'banned');assert.equal(M.CARD_CATALOG['Dockside Extortionist'].deckImportEligible,true);
