...
import {sourceDir,outputDir} from './import-brc-onc-moc-sld-precons.mjs';
...
const variants=[
 ['Wreck Hunter','Powerstone','Powerstone'],
 ['Brimaz, Blight of Oreskos','Incubator // Phyrexian','MOC Incubator'],
 ['Vishgraz, the Doomhive','Phyrexian Mite','ONC Mite'],
 ['Gimbal, Gremlin Prodigy','Gremlin','MOC Gremlin'],
 ['Urza, Chief Artificer','Construct','BRC Construct'],
 ['Farid, Enterprising Salvager','Scrap','Scrap'],
 ['Wildfire Awakener','Elemental','MOC Wildfire Elemental'],
 ["Goldwardens' Gambit",'Rebel','ONC Rebel'],
 ['Everquill Phoenix','Feather','Feather'],
 ['Nissa, Vastwood Seer // Nissa, Sage Animist','Ashaya, the Awoken World','Ashaya, the Awoken World']
];
...
for(const[source,name,alias]of variants){
 ...
 if(alias==='MOC Incubator')metadata.set('MOC Phyrexian',{...tokens[alias],...tokens[alias].card_faces[1]});
}
...
const names=new Set([...records.flatMap(r=>[r.requestedName.split(' // ')[0],...(r.faces||[]).map(f=>f.name)]),...variants.map(r=>r[2]),'MOC Phyrexian']);
...
const canonicalAliases = {'Phyrexian Mite':'ONC Mite'};
