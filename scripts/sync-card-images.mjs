...
function manifestSource(cardPaths, artPaths, missing) {
  ...
  const missingComment = missing.length
    ? `// No Scryfall print was available for ${missing.length} names; they use the local card back.\n`
    : '';
  return `'use strict';\nvar MTG = globalThis.MTG || (globalThis.MTG = {});\n\n` +
    ...
    `  if (variant === 'art') {\n` +
    `    return MTG.CARD_ART_PATHS[face] || MTG.CARD_IMAGE_PATHS[face] || MTG.CARD_IMAGE_PLACEHOLDER;\n` +
    `  }\n` +
    `  return MTG.CARD_IMAGE_PATHS[face] || MTG.CARD_IMAGE_PLACEHOLDER;\n` +
    `};\n`;
}
