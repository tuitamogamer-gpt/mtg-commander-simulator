...
import {sourceDir,outputDir} from './import-fdc-precons.mjs';
...
const variants=[['The Elder Dragon War','Dragon','FDC Dragon']];
...
for(const[variant,wanted]of [['normal',names],['art_crop',lists.map(d=>d.commander)]])for(const name of wanted){
...
fs.writeFileSync(reportFile,JSON.stringify({date:'2026-09-26',source:'Pinned Scryfall card and linked token metadata',added,addedCount:added.length,canonicalAliases,sourceCardsWithLocalImages:records.length,tokenVariants:variants.map(([source,name,alias])=>({source,name,alias,scryfallId:tokens[alias].id})),existingImagesUnchanged:true},null,2)+'\n');
