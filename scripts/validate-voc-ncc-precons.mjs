...
import {sourceDir,buildIntake} from './import-voc-ncc-precons.mjs';
const base='4a7a901',sha=s=>crypto.createHash('sha256').update(s).digest('hex');
...
assert.equal(Object.keys(current.cards).length-Object.keys(previous.cards).length,165);assert.equal(current.decks.length-previous.decks.length,10);
...
for(const d of i.decks){const validation=M.validateImportedDeck(M.parseDeckText(fs.readFileSync(sourceDir+'/'+d.source.export,'utf8')),{name:d.name});if(d.name==='Upgrades Unleashed'){assert.equal(validation.ok,false);assert.equal(validation.errors.length,1);assert.equal(validation.errors[0].code,'singleton');assert.equal(validation.errors[0].card,'Mossfire Valley');}else assert.equal(validation.ok,true,d.name+': '+JSON.stringify(validation.errors));}
