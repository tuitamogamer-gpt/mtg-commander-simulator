...
import {sourceDir,buildIntake} from './import-afc-mic-precons.mjs';
const base='030c0de',sha=s=>crypto.createHash('sha256').update(s).digest('hex');
...
assert.equal(Object.keys(current.cards).length-Object.keys(previous.cards).length,92);assert.equal(current.decks.length-previous.decks.length,5);
