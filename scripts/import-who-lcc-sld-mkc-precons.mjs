...
export const sourceDir=path.resolve(path.dirname(fileURLToPath(import.meta.url)),'../reports/decks/precon-who-lcc-sld-mkc-2026-09-12');
export const outputDir=path.resolve(sourceDir,'../../../output/precon-who-lcc-sld-mkc-2026-09-12');
export const precons=[
  {name:'Paradox Power',commander:'The Thirteenth Doctor',partner:'Yasmin Khan',set:'WHO',moxfield:'l-e7-HDtsk2hXy5HK7ASuQ'},
  {name:'Masters of Evil',commander:'Davros, Dalek Creator',set:'WHO',moxfield:'4gxofFdLD0OQleQer2rVbg'},
  {name:'Blast from the Past',commander:'The Fourth Doctor',partner:'Sarah Jane Smith',set:'WHO',moxfield:'EDdBpyjUFUOBH2ZoOzAkxA'},
  {name:'Veloci-Ramp-Tor',commander:'Pantlaza, Sun-Favored',set:'LCC',moxfield:'viXU1nCwOkmTU5FGQu8MFw'},
  {name:'Explorers of the Deep',commander:'Hakbal of the Surging Soul',set:'LCC',moxfield:'lnTvk7dGp0KzsvxIMxPDJg'},
  {name:'Blood Rites',commander:'Clavileño, First of the Blessed',set:'LCC',moxfield:'OpCAH5tOAk6UznEeBm2y2Q'},
  {name:'Ahoy Mateys',commander:'Admiral Brass, Unsinkable',set:'LCC',moxfield:'JvChR5Ti-0WW5QE3PD29hA'},
  {name:'Raining Cats and Dogs',commander:'Rin and Seri, Inseparable',set:'SLD',moxfield:'G0y9ltIwoUysUSoAYtyFfA'},
  {name:'Revenant Recon',commander:'Mirko, Obsessive Theorist',set:'MKC',moxfield:'iGDLYBkYqEGQOinzm-807g'},
  {name:'Deadly Disguise',commander:'Kaust, Eyes of the Glade',set:'MKC',moxfield:'c9xQjfvGwkCMFmQWcdOKVw'}
].map(row=>({...row,slug:row.name.toLowerCase().replace(/[^a-z0-9]+/g,'-')}));
const officialUrls=[
  {set:'WHO',file:'who.html',url:'https://magic.wizards.com/en/news/announcements/magic-the-gathering-doctor-who-commander-decklists'},
  {set:'LCC',file:'lcc.html',url:'https://magic.wizards.com/en/news/announcements/the-lost-caverns-of-ixalan-commander-decklists'},
  {set:'SLD',file:'sld.html',url:'https://magic.wizards.com/en/news/announcements/secret-lair-commander-deck-raining-cats-and-dogs'},
  {set:'MKC',file:'mkc.html',url:'https://magic.wizards.com/en/news/announcements/murders-at-karlov-manor-commander-decklists'}
];
const sha=t=>crypto.createHash('sha256').update(t).digest('hex');
// Normalize punctuation, combined identities and the official shorthand for Dusk // Dawn.
const canonical=n=>n.replace(/<[^>]*>/g,'').replaceAll('Æ','Ae').replaceAll('æ','ae').replaceAll('’',"'").replace(/\s*\/{2,3}\s*/g,' // ').trim().replace(/^Ransom Note \((?:Surveil|Morph)\)$/,'Ransom Note').replace(/^Dusk$/,'Dusk // Dawn').replace(/^Armont, the Redeemer$/,'Syr Armont, the Redeemer').replace(/^(.+) \/\/ \1$/,'$1');
...
async function sources(){
  ...
  if(official.length!==13)throw Error('Expected thirteen source lists, got '+official.length);
  ...
  write(path.join(sourceDir,'decklists.json'),{retrievedOn:'2026-09-12',selection:'After Timey-Wimey, by release order and pinned-index order within each edition: three remaining Doctor Who decks, four Lost Caverns of Ixalan decks, Raining Cats and Dogs, Revenant Recon and Deadly Disguise. Deep Clue Sea is already present; Blame Game falls after this batch and retains its previous exclusion. Preserve every original card regardless of legality. No substitutions or alternate-edition duplicates.',moxfieldAccess:'The pinned index supplies queue links. These are verified Wizards / MTGJSON lists, not direct Moxfield exports.',normalization:'Flip, Adventure and double-faced cards use their front names for source comparison and resolve to existing canonical aliases during intake; split cards retain their combined names. The source labels Ransom Note (Surveil) and Ransom Note (Morph) resolve to the same Ransom Note identity. Quantity spacing in the official HTML is normalized. Time Lord is a single creature type. Full Scryfall faces are preserved in oracle.json.',decks});
}
