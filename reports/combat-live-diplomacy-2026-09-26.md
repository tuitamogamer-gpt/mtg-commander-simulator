...
The contention fix extends lock acquisition past the five-second lease, reports temporary refusal separately from invalid gameplay, and retries the same action or join with bounded one-to-two-second backoff. Busy retries preserve queue order and do not send gameplay before joining. Exhausted retries stop and reject queued work rather than sending dependent actions after a rejected request. Seven server replay tests and eleven client recovery tests cover these paths. That revision's `npm run test:server` passes all 27 checks, and syntax/whitespace checks pass. Production evidence uses distinct files so previous failures remain reviewable.

The next run on `8cd7a26` passed eight groups and crossed two natural host socket closures, but failed during a deliberate guest reload: the returning guest saw turn four while existing clients retained turn five, and another guest received a superseded-connection close. No browser exception was captured. That failure remains in `production-recovery-live-four-r2/`. Review found two concrete server races consistent with this evidence: a worker could write after its Redis lock expired, and an older broadcast could incorrectly revoke a newer connection. The captured production evidence alone does not distinguish their exact contribution.

Redis writes now check the lock owner atomically with the write, active leases renew, and a lost lease defers the unchanged action for retry. Cleanup failure cannot reject an already committed action. Broadcasts older than a client's accepted revision are discarded before checking connection ownership. Accepted joins subscribe immediately after storage succeeds, so newer states cannot be missed during delayed publication. Store tests simulate an expired holder resuming after another worker changes the connection, game view and receipts; server tests cover delayed broadcasts, genuine seat replacement and delayed join publication. All 33 focused server/client/store checks and syntax/whitespace checks pass. The deterministic store fixture mocks Redis operations; the final production four-client gate exercises the actual Redis service and records sanitized revision, phase and socket metadata. The gate rejects revision regressions and unexpected seat replacement as well as requiring post-reconnect gameplay.
